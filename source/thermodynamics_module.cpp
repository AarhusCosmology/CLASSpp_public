...
int ThermodynamicsModule::thermodynamics_at_z(
  ...
  /** - interpolate in table with array_interpolate_spline() (normal
    ...
    if (((pth->reio_parametrization == reio_half_tanh) && (z < 2 * z_reionization_)) ||
      ...
      class_call(array_interpolate_linear(const_cast<double*>(z_table_.data()),
                                          tt_size_,
    ...
    /* in the "normal" case, use spline interpolation */
      ...
      if (inter_mode == inter_closeby_) {
        class_call(array_interpolate_spline_growing_closeby(const_cast<double*>(z_table_.data()),
                                                            tt_size_,
                                                            const_cast<double*>(
...
int ThermodynamicsModule::thermodynamics_init() {
  ...
  class_test((pth->annihilation < 0), error_message_, "annihilation parameter cannot be negative");
  ...
  class_test(_not4_ == 0., error_message_, "stop to avoid division by zero");
  class_test(YHe_ == 1., error_message_, "stop to avoid division by zero");
  ...
  class_call(thermodynamics_indices(preco, preio), error_message_, error_message_);
  ...
  class_call(thermodynamics_merge_reco_and_reio(preco, preio), error_message_, error_message_);
  ...
  for (index_tau = 0; index_tau < tt_size_; index_tau++) {
    class_call(background_module_->background_tau_of_z(z_table_[index_tau],
                                                       tau_table.data() + index_tau),
  ...
  for (index_tau = 0; index_tau < tt_size_; index_tau++) {
    class_call(background_module_->background_at_tau(tau_table[index_tau],
                                                     pba->normal_info,
                                                     pba->inter_closeby,
                                                     &last_index_back,
                                                     pvecback.data()),
  ...
  if (pth->compute_damping_scale == _TRUE_) {
    ...
    for (index_tau = 0; index_tau < tt_size_; index_tau++) {
      ...
      class_call(background_module_->background_at_tau(tau_table_growing[index_tau],
                                                       pba->normal_info,
                                                       pba->inter_closeby,
                                                       &last_index_back,
                                                       pvecback.data()),
  ...
  class_call(background_module_->background_at_tau(tau_rec_,
                                                   pba->long_info,
                                                   pba->inter_normal,
                                                   &last_index_back,
                                                   pvecback.data()),
  ...
  if (all_species_.count("IDM_DR_IDR") > 0) {
    ...
    /* case of idr alone without idm_dr */
      ...
      class_call(background_module_->background_tau_of_z(z_table_[index_tau], &tau),
                 background_module_->error_message_,
                 error_message_) tau_idm_dr_fs = tau;
      tau_idr_free_streaming_                  = tau;
    }
  }
  ...
  class_call(background_module_->background_at_tau(tau_star_,
                                                   pba->long_info,
                                                   pba->inter_normal,
                                                   &last_index_back,
                                                   pvecback.data()),
  ...
  class_call(background_module_->background_at_tau(tau_d_,
                                                   pba->long_info,
                                                   pba->inter_normal,
                                                   &last_index_back,
                                                   pvecback.data()),
...
int ThermodynamicsModule::thermodynamics_helium_from_bbn() {
  ...
  class_call(background_module_->background_at_tau(tau_bbn,
                                                   pba->long_info,
                                                   pba->inter_normal,
                                                   &last_index,
                                                   pvecback.data()),
...
int ThermodynamicsModule::thermodynamics_reionization_function(double z,
  ...
  if (pth->reio_parametrization == reio_inter) {
    ...
    if (z > preio->reionization_parameters[preio->index_reio_first_z + preio->reio_num_z - 1]) {
      *xe = preio->reionization_parameters[preio->index_reio_first_xe + preio->reio_num_z - 1];
      ...
      class_test(z < z_min, error_message_, "");

      class_test(z > z_max, error_message_, "");
  ...
  class_test(0 == 0,
             error_message_,
             "value of reio_parametrization=%d unclear",
...
int ThermodynamicsModule::thermodynamics_reionization(recombination* preco,
  ...
  if ((pth->reio_parametrization == reio_camb) || (pth->reio_parametrization == reio_half_tanh)) {
    ...
    if (pth->reio_z_or_tau == reio_z) {
      ...
      class_call(thermodynamics_get_xe_before_reionization(
                     preco,
                     preio->reionization_parameters[preio->index_reio_start],
                     &(preio->reionization_parameters[preio->index_reio_xe_before])),
                 error_message_,
  ...
  if (pth->reio_parametrization == reio_bins_tanh) {
    ...
    */
    ...
    class_call(thermodynamics_get_xe_before_reionization(
                   preco,
                   preio
                       ->reionization_parameters[preio->index_reio_first_z + preio->reio_num_z - 1],
                   &(preio->reionization_parameters[preio->index_reio_first_xe + preio->reio_num_z -
                                                    1])),
               error_message_,
               error_message_);
  ...
  class_test(0 == 0, error_message_, "value of reio_z_or_tau=%d unclear", pth->reio_z_or_tau);
}
...
int ThermodynamicsModule::thermodynamics_reionization_sample(recombination* preco,
  ...
  /** - --> get \f$ X_e \f$ */
  double xe;
  class_call(thermodynamics_reionization_function(z, preio, &xe), error_message_, error_message_);
...
int ThermodynamicsModule::thermodynamics_recombination_with_hyrec(recombination* preco,
  ...
  param.nH0             = 11.223846333047 * param.obh2 *
                          (1. - param.Y); /* number density of hydrogen today in m-3 */
  ...
  buf_size = (2 * NTR + NTM + 2 * NTR * NTM + 2 * param.nz) * sizeof(double) +
             2 * NTM * sizeof(double*);
  ...
  L2s1s_current = 0.;
  for (int b = 0; b < NSUBLYA; b++)
    L2s1s_current += twog_params.A2s_tab[b];
  for (int b = 0; b < NSUBLYA; b++)
    twog_params.A2s_tab[b] *= L2s1s / L2s1s_current;
  ...
  preco->YHe  = YHe_;
  preco->Nnow = 3. * preco->H0 * preco->H0 * pba->Omega0_b * (1. - preco->YHe) /
                (8. * _PI_ * _G_ * _m_H_);
    ...
    */
    ...
    preco->recombination_table[(Nz - i - 1) * preco->re_size + preco->index_re_cb2] =
        ...
        (1. - rec_dTmdlna(xe,
                          ...
                          Hz,
                          param.fHe,
                          param.nH0 * pow((1 + z), 3) * 1e-6,
                          energy_injection_rate(&param, z)) /
                  Tm / 3.);
  ...
  class_stop(error_message_,
             "you compiled without including the HyRec code, and now wish to use it. Either set "
             "the input parameter 'recombination' to something else than 'HyRec', or recompile "
...
int ThermodynamicsModule::thermodynamics_recombination_with_recfast(recombination* preco,
  ...
  for (int i = 0; i < Nz; i++) {
    ...
    /** - --> second approximation: first Helium recombination (analytic approximation) */
      ...
      rhs = exp(1.5 * log(preco->CR * preco->Tnow / (1. + z)) -
      ...
      if (z > ppr->recfast_z_He_1 - ppr->recfast_delta_z_He_1) {
        ...
        x0_new = 0.5 * (sqrt(pow((rhs - 1. - preco->fHe), 2) + 4. * (1. + 2. * preco->fHe) * rhs) -
                        (rhs - 1. - preco->fHe));
      ...
      /* transition finished */
        ...
        x0 = 0.5 * (sqrt(pow((rhs - 1. - preco->fHe), 2) + 4. * (1. + 2. * preco->fHe) * rhs) -
                    (rhs - 1. - preco->fHe));
      }
    ...
    /** - --> third approximation: first Helium recombination completed */
      ...
      if (z > ppr->recfast_z_He_2 - ppr->recfast_delta_z_He_2) {
        rhs         = exp(1.5 * log(preco->CR * preco->Tnow / (1. + z)) -
                      ...
                      preco->Nnow;
        x0_previous = 0.5 *
                      (sqrt(pow((rhs - 1. - preco->fHe), 2) + 4. * (1. + 2. * preco->fHe) * rhs) -
                       (rhs - 1. - preco->fHe));
        x0_new      = 1. + preco->fHe;
        /* get s from -1 to 1 */
    ...
    preco->recombination_table[(Nz - i - 1) * preco->re_size + preco->index_re_wb] =
        _k_B_ / (_c_ * _c_ * _m_H_) *
        (1. + (1. / _not4_ - 1.) * preco->YHe + x0 * (1. - preco->YHe)) * y[2];
...
int ThermodynamicsModule::thermodynamics_derivs_with_recfast_member(
  ...
  if (ppr->recfast_Hswitch == _TRUE_)
    K *= 1. +
         ...
         ppr->recfast_AGauss2 *
             exp(-pow((log(1. + z) - ppr->recfast_zGauss2) / ppr->recfast_wGauss2, 2));
  ...
  if (x_H > ppr->recfast_x_H0_trigger)
    ...
    if (x_H < ppr->recfast_x_H0_trigger2) {
      C = (1. + K * _Lambda_ * n * (1. - x_H)) /
          (1. / preco->fu + K * _Lambda_ * n * (1. - x_H) / preco->fu + K * Rup * n * (1. - x_H));
    }
    else {
      C = 1.;
    }
