    ...
    for (int index_k = 0; index_k < k_size_[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          class_store_double(dataptr, tk[index_tp_delta_idm_drmd_], has_source_delta_idm_drmd_, storeidx);
        ...
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          class_store_double(dataptr, tk[index_tp_theta_idm_drmd_], has_source_theta_idm_drmd_, storeidx);
...
int PerturbationsModule::perturb_output_titles(enum file_format output_format, char titles[_MAXTITLESTRINGLENGTH_]) const {
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles, "d_idm_drmd", pba->has_idm_drmd);
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles, "t_idm_drmd", pba->has_idm_drmd);
  ...
  */
  ...
  while (tau < background_module_->conformal_age_) {
    ...
    if (has_cmb_ == _TRUE_) {
      ...
      double a_primeprime_over_a = pvecback[background_module_->index_bg_H_prime_]*pvecback[background_module_->index_bg_a_] + 2.*a_prime_over_a*a_prime_over_a;
...
int PerturbationsModule::perturb_workspace_init(int index_md, perturb_workspace* ppw) {
  ...
  if (_scalars_) {
    ...
    class_define_index(ppw->index_ap_tca_idm_drmd, pba->has_idm_drmd && pba->has_idr_drmd, index_ap, 1); /* DRMD */
  ...
  if (_scalars_) {
    ...
    if ((pba->has_idr_drmd == _TRUE_) && (pba->has_idm_drmd == _TRUE_)) { /* DRMD : Only if both fluids are present, does it make sense to have the tight coupling approximation.*/
...
int PerturbationsModule::perturb_prepare_k_output() {
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      class_store_columntitle(scalar_titles_, "delta_idr_drmd", pba->has_idr_drmd);
      class_store_columntitle(scalar_titles_, "theta_idr_drmd", pba->has_idr_drmd);
      /* Interacting dark matter (DRMD) */
      class_store_columntitle(scalar_titles_, "delta_idm_drmd", pba->has_idm_drmd);
      class_store_columntitle(scalar_titles_, "theta_idm_drmd", pba->has_idm_drmd);
      /* Cold dark matter */
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          {
            if ((interval_approx[index_switch - 1][ppw->index_ap_tca_idm_drmd] == (int)tca_idm_drmd_on) &&
                (interval_approx[index_switch][ppw->index_ap_tca_idm_drmd] == (int)tca_idm_drmd_off))
              fprintf(stdout, "Mode k=%e: will switch off dark tight-coupling approximation at tau=%e\n", k, interval_limit[index_switch]);
          }
...
int PerturbationsModule::perturb_vector_init(
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for(int n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++){
        ...
        if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_off){
          ...
          ppv->q_size_ncdm[n_ncdm] = ncdm_->q_size_ncdm_[n_ncdm];
        }
        else{
        ...
        for (int index_q = 0; index_q < ppv->q_size_ncdm[n_ncdm]; index_q++) {
          ppv->index_ncdm_[n_ncdm].push_back(index_pt + index_q*(ppv->l_max_ncdm[n_ncdm] + 1));
        }
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for(int n_ncdm = 0; n_ncdm < ppv-> N_ncdm; n_ncdm++){
        for(int index_q=0; index_q < ppv->q_size_ncdm[n_ncdm]; index_q++){
          for(int l=0; l<=ppv->l_max_ncdm[n_ncdm]; l++){
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if ((pba->has_idm_drmd == _TRUE_) && (pba->has_idr_drmd == _TRUE_))
      {
        class_test((pa_old[ppw->index_ap_tca_idm_drmd] == (int)tca_idm_drmd_off) && (ppw->approx[ppw->index_ap_tca_idm_drmd] == (int)tca_idm_drmd_on),
                   error_message_,
                   "at tau=%g: the dark tight-coupling approximation can be switched off, not on (DRMD)", tau);
      }
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        if (pba->has_idr_drmd == _TRUE_)
        {
          ppv->y[ppv->index_pt_delta_idr_drmd] =
              ppw->pv->y[ppw->pv->index_pt_delta_idr_drmd];
          ppv->y[ppv->index_pt_theta_idr_drmd] =
              ppw->pv->y[ppw->pv->index_pt_theta_idr_drmd];
        }
        ...
        if (pba->has_idm_drmd == _TRUE_)
        {
          ppv->y[ppv->index_pt_delta_idm_drmd] =
              ppw->pv->y[ppw->pv->index_pt_delta_idm_drmd];
          ppv->y[ppv->index_pt_theta_idm_drmd] =
              ppw->pv->y[ppw->pv->index_pt_theta_idm_drmd];
        }
      ...
      if (pba->has_ncdm == _TRUE_) {
        ...
        if ((pa_old[ppw->index_ap_ncdmfa] == (int)ncdmfa_off) && (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on)) {
          ...
          for (int n_ncdm = 0; n_ncdm < ppv->N_ncdm; n_ncdm++) {
            ...
            int ncdm_l_size = ppv->l_max_ncdm[n_ncdm] + 1;
            double rho_plus_p_ncdm = ppw->pvecback[background_module_->index_bg_rho_ncdm1_ + n_ncdm] + ppw->pvecback[background_module_->index_bg_p_ncdm1_ + n_ncdm];
            for (int l = 0; l <= 2; l++) {
              ppv->y[ppv->index_pt_psi0_ncdm1+ncdm_l_size*n_ncdm+l] = 0.0;
            }
            double factor = ncdm_->factor_ncdm_[n_ncdm]*pow(pba->a_today/a, 4);
            ...
            switch (ncdm_->ncdm_types_[n_ncdm]) {
              case NCDMType::standard:
                for (int index_q = 0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++) {
                  const int index_pt = ppw->pv->index_ncdm_[n_ncdm][index_q];
                  double w0 = ncdm_->w_ncdm_[n_ncdm][index_q];
                  double q = ncdm_->q_ncdm_[n_ncdm][index_q];
                  double epsilon = sqrt(q*q + a*a*ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]);
              ...
              case NCDMType::decay_dr:
                ...
                std::tie(delta, theta, shear) = RescaledNCDMPerturbations(n_ncdm, a, k, ppw);
                break;
            }
   ...
   of variables */
      ...
      if ((pba->has_idm_drmd == _TRUE_) && (pba->has_idr_drmd == _TRUE_))
      {
        ...
        if ((pa_old[ppw->index_ap_tca_idm_drmd] == (int)tca_idm_drmd_on) && (ppw->approx[ppw->index_ap_tca_idm_drmd] == (int)tca_idm_drmd_off))
        {
          double a = ppw->pvecback[background_module_->index_bg_a_];

          if (ppt->perturbations_verbose > 2)
            fprintf(stdout, "Mode k=%e: switch off dark tight coupling approximation (DRMD) at tau=%e and z=%e\n", k, tau,1./a - 1.);
          ...
          ppv->y[ppv->index_pt_theta_idm_drmd] = theta_idr - 1./(1. + Rint)*(k*k/4. * delta_idr + a_prime_over_a*theta_idr);
        }
      }
    }
  ...
  if (_scalars_) {
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == index_ic_ad_)) {
      ...
      /* interacting dark matter (DRMD) */
      /* Here we set the inital conditions. */
      /* DRMD: These initial conditions take into acccount the absence of shear stress (different from _ur component!). These are the correct conditions deep in rad. domination. To do: add corrections in om = aini/aeq if one wants to initialize perturbations later. */
      if (pba->has_idr_drmd == _TRUE_)
      {
        ppw->pv->y[ppw->pv->index_pt_delta_idr_drmd] = ppw->pv->y[ppw->pv->index_pt_delta_g];
        ppw->pv->y[ppw->pv->index_pt_theta_idr_drmd] = ppw->pv->y[ppw->pv->index_pt_theta_g];
      }

      if (pba->has_idm_drmd == _TRUE_)
      {
        ppw->pv->y[ppw->pv->index_pt_delta_idm_drmd] = 3./4.*ppw->pv->y[ppw->pv->index_pt_delta_g]; /* idm_DRMD density */

        if (pba->has_idr_drmd == _TRUE_)
        {
          if (ppw->approx[ppw->index_ap_tca_idm_drmd] == (int)tca_idm_drmd_on)
          {
            ppw->pv->y[ppw->pv->index_pt_theta_idm_drmd] = ppw->pv->y[ppw->pv->index_pt_theta_idr_drmd]; // DRMD: This is the tight-coupling condition, which only makes sense for strong (G/(aH)>>1 coupling.
          }
          else
          {
        ...
        {
          ppw->pv->y[ppw->pv->index_pt_theta_idm_drmd] = 0.0; // DRMD: In the absence of interactions, velocity vanishes initially in the synchronous gauge.
        }
      }
    ...
    if (ppt->gauge == newtonian) {
      ...
      class_test((pba->has_idr_drmd == _TRUE_),
                 error_message_,
                 "Use synchronous gauge for the DRMD implementation as Netwonian gauge has not been tested!");
      ...
      */
      ...
      if (pba->has_idm_drmd == _TRUE_) {
        delta_cdm += ppw->pvecback[background_module_->index_bg_rho_idm_drmd_]*ppw->pv->y[ppw->pv->index_pt_delta_idm_drmd];
      }
      else
        delta_cdm=0.;
      ...
      double delta_tot = (fracg*ppw->pv->y[ppw->pv->index_pt_delta_g]+fracnu*delta_ur+rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_delta_b]+fraccdm*delta_cdm))/(1.+rho_m_over_rho_r);

      double velocity_tot = ((4./3.)*(fracg*ppw->pv->y[ppw->pv->index_pt_theta_g]+fracnu*theta_ur) + rho_m_over_rho_r*fracb*ppw->pv->y[ppw->pv->index_pt_theta_b])/(1.+rho_m_over_rho_r);

     
      if (pba->has_idm_drmd == _TRUE_)
      {
        delta_tot += rho_m_over_rho_r*fracidm_drmd*ppw->pv->y[ppw->pv->index_pt_delta_idm_drmd]/(1. + rho_m_over_rho_r);
        velocity_tot += rho_m_over_rho_r*fracidm_drmd*ppw->pv->y[ppw->pv->index_pt_theta_idm_drmd]/(1. + rho_m_over_rho_r);
      }
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_dr == _TRUE_)  || (pba->has_idr == _TRUE_)) {
        ...
        if (pba->has_ncdm_decay_dr == _TRUE_) {
          throw std::invalid_argument("Newtonian gauge not implemented with decaying NCDM species.");
          /*
    ...
    if (pba->has_ncdm == _TRUE_) {
      for (int n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++){
        ...
        for (int index_q = 0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++) {
          const int idx = ppw->pv->index_ncdm_[n_ncdm][index_q];
          double q = ncdm_->q_ncdm_[n_ncdm][index_q];
          double epsilon = sqrt(q*q + a*a*ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]);
          double dlnf0_dlnq;
          switch (ncdm_->ncdm_types_[n_ncdm]) {
            case NCDMType::standard: {
              dlnf0_dlnq = ncdm_->dlnf0_dlnq_ncdm_[n_ncdm][index_q];
              break;
            }
            case NCDMType::decay_dr: {
              // If the current species can decay, make sure to get the time-dependent distribution function
              dlnf0_dlnq = ppw->pvecback[background_module_->index_bg_dlnfdlnq_ncdm_decay_dr1_ + ncdm_->decay_dr_map_[n_ncdm].q_offset + index_q];
              break;
            }
          }
...
int PerturbationsModule::perturb_total_stress_energy(int index_md, double k, double* y, perturb_workspace* ppw) {
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on){
        ...
        for(int n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          double rho_ncdm_bg = ppw->pvecback[background_module_->index_bg_rho_ncdm1_ + n_ncdm];
          double p_ncdm_bg = ppw->pvecback[background_module_->index_bg_p_ncdm1_ + n_ncdm];
          double pseudo_p_ncdm = ppw->pvecback[background_module_->index_bg_pseudo_p_ncdm1_ + n_ncdm];
          double rho_plus_p_ncdm = rho_ncdm_bg + p_ncdm_bg;
          ...
          switch (ncdm_->ncdm_types_[n_ncdm]) {
            ...
            case NCDMType::decay_dr:
              ...
              std::tie(w_ncdm, pseudo_p_over_p) = ncdm_->GetRescaledParameters(n_ncdm, a, ppw->pvecback + background_module_->index_bg_lnf_ncdm_decay_dr1_);
        ...
        for(int n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          if (ncdm_->ncdm_types_[n_ncdm] == NCDMType::decay_dr) {
            must_rescale = true;
          }
          for (int index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q ++) {
            const int idx = ppw->pv->index_ncdm_[n_ncdm][index_q];
            ...
            switch (ncdm_->ncdm_types_[n_ncdm]) {
              case NCDMType::standard:
                w0 = ncdm_->w_ncdm_[n_ncdm][index_q];
                break;
              case NCDMType::decay_dr:
                double dq = ncdm_->decay_dr_map_[n_ncdm].dq[index_q];
                double f_q = exp(ppw->pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + ncdm_->decay_dr_map_[n_ncdm].q_offset + index_q]);
                w0 = dq*f_q;
                break;
            }
            double q = ncdm_->q_ncdm_[n_ncdm][index_q];
            double q2 = q*q;
            double epsilon = sqrt(q2 + ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]*a2);
          ...
          if ((has_source_delta_ncdm_ == _TRUE_) || (has_source_theta_ncdm_ == _TRUE_) || (has_source_delta_m_ == _TRUE_)) {
            if (must_rescale) {
              ...
              std::tie(delta, theta, shear) = RescaledNCDMPerturbations(n_ncdm, a, k, ppw);
              ppw->delta_ncdm[n_ncdm] = delta;
              ppw->theta_ncdm[n_ncdm] = theta;
              ppw->shear_ncdm[n_ncdm] = shear;
            }
            else {
    ...
    */
	...
	ppw->delta_p_fld = pba->cs2_fld * ppw->delta_rho_fld + (pba->cs2_fld-ca2_fld)*(3*a_prime_over_a*ppw->rho_plus_p_theta_fld/k/k);
        ...
        double c_gamma_k_H_square = pow(pba->c_gamma_over_c_fld*k/a_prime_over_a,2)*pba->cs2_fld;
  ...
  ppw->delta_rho_fld = -2./3.*k2*s2sq/a2*Gamma_fld - 3*a_prime_over_a/k2*ppw->rho_plus_p_theta_fld;
	...
	double X_prime = -2*X*(a_prime_over_a + ppw->pvecback[background_module_->index_bg_H_prime_]/ppw->pvecback[background_module_->index_bg_H_]);
	...
	double Z_prime = Z*(ppw->pvecback[background_module_->index_bg_H_prime_]/ppw->pvecback[background_module_->index_bg_H_] - a_prime_over_a);
	/** Construct theta_t and its derivative from the Euler equation */
	double theta_t = ppw->rho_plus_p_theta/ppw->rho_plus_p_tot;
	double theta_t_prime = -a_prime_over_a*theta_t-(p_t_prime*theta_t-k2*ppw->delta_p +k2*ppw->rho_plus_p_shear)/ppw->rho_plus_p_tot+metric_euler;
	double S = ppw->S_fld;
	double S_prime = -Z_prime/Z*S+1./Z*(rho_fld_prime+p_fld_prime)*(theta_t+k2*alpha)+1./Z*(rho_fld+p_fld)*(theta_t_prime+k2*alpha_prime);
	/** Analytic derivative of the equation for ppw->rho_plus_p_theta_fld above. */
	double rho_plus_p_theta_fld_prime = Z_prime*(S-1./(1.+Y)*(S/(1.+1./X)+Gamma_fld*X)) +
...
int PerturbationsModule::perturb_sources_member(double tau, double* y, double* dy, int index_tau, void* parameters_and_workspace, ErrorMsg error_message) {
  ...
  double a_prime_over_a = pvecback[background_module_->index_bg_a_]*pvecback[background_module_->index_bg_H_]; /* (a'/a)=aH */
  double a_prime_over_a_prime = pvecback[background_module_->index_bg_H_prime_]*pvecback[background_module_->index_bg_a_] +
  ...
  if (_scalars_) {
    ...
    if (has_source_H_T_Nb_prime_ == _TRUE_) {
      double rho_plus_p_tot = (pvecback[background_module_->index_bg_rho_tot_] + pvecback[background_module_->index_bg_p_tot_]);
    ...
    /* delta_idm_drmd */
    if (has_source_delta_idm_drmd_ == _TRUE_)
    {
      _set_source_(index_tp_delta_idm_drmd_) = y[ppw->pv->index_pt_delta_idm_drmd] + 3.*a_prime_over_a*theta_over_k2; // N-body gauge correction
    }
    ...
    if (has_source_delta_dr_ == _TRUE_) {
      double r_dr = pow(a2_rel/pba->H0, 2)*pvecback[background_module_->index_bg_rho_dr_];
    ...
    /* delta_idr_drmd */
    if (has_source_delta_idr_drmd_ == _TRUE_)
    {
      _set_source_(index_tp_delta_idr_drmd_) = y[ppw->pv->index_pt_delta_idr_drmd] + 4.*a_prime_over_a*theta_over_k2; // N-body gauge correction
    }

    /* delta_ncdm1 */
    if (has_source_delta_ncdm_ == _TRUE_) {
      for (int index_tp = index_tp_delta_ncdm1_; index_tp < index_tp_delta_ncdm1_ + pba->N_ncdm; index_tp++) {
    ...
    /* theta_idm_drmd */
    if (has_source_theta_idm_drmd_ == _TRUE_)
    {
      _set_source_(index_tp_theta_idm_drmd_) = y[ppw->pv->index_pt_theta_idm_drmd] + theta_shift; // N-body gauge correction
    }
    ...
    if (has_source_theta_scf_ == _TRUE_) {
      ...
      double rho_plus_p_theta_scf = 1./3.*
    ...
    if (has_source_theta_ncdm_ == _TRUE_) {
      for (int index_tp = index_tp_theta_ncdm1_; index_tp < index_tp_theta_ncdm1_ + pba->N_ncdm; index_tp++) {
...
int PerturbationsModule::perturb_print_variables_member(double tau, double* y, double* dy, void* parameters_and_workspace, ErrorMsg error_message) {
  ...
  double a2 = a*a;
  double H = pvecback[background_module_->index_bg_H_];
  ...
  /* Physics variables: used in both _scalars_ and _tensors_ blocks */
  double delta_g = 0., theta_g = 0.;
  double shear_g = 0., pol0_g = 0., pol1_g = 0., pol2_g = 0.;
  ...
  if (_scalars_) {
    ...
    if (pba->has_idm_drmd == _TRUE_) 
    {
      delta_idm_drmd = y[ppw->pv->index_pt_delta_idm_drmd];
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on){
        // The perturbations are evolved integrated:
        for(int n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++){
          double rho_ncdm_bg = pvecback[background_module_->index_bg_rho_ncdm1_ + n_ncdm];
          double p_ncdm_bg = pvecback[background_module_->index_bg_p_ncdm1_ + n_ncdm];
          double pseudo_p_ncdm = pvecback[background_module_->index_bg_pseudo_p_ncdm1_ + n_ncdm];
        ...
        for(int n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          if (must_rescale) {
            std::tie(delta_ncdm[n_ncdm], theta_ncdm[n_ncdm], shear_ncdm[n_ncdm]) = RescaledNCDMPerturbations(n_ncdm, a, k, ppw);
          }
          else {
    ...
    std::vector<double> delta_dr, theta_dr, shear_dr;
    if (pba->has_dr == _TRUE_) {
      for (int n_dr = 0; n_dr < pba->N_decay_dr; n_dr++) {
        double r_dr = pow(pvecback[background_module_->index_bg_a_]*pvecback[background_module_->index_bg_a_]/pba->H0, 2)*pvecback[background_module_->index_bg_rho_dr_species_ + n_dr];
        delta_dr.emplace_back(y[ppw->pv->index_pt_F0_dr_species + n_dr*(ppw->pv->l_max_dr + 1)]/r_dr);
        theta_dr.emplace_back(y[ppw->pv->index_pt_F0_dr_species + n_dr*(ppw->pv->l_max_dr + 1) + 1]*3./4.*k/r_dr);
        shear_dr.emplace_back(y[ppw->pv->index_pt_F0_dr_species + n_dr*(ppw->pv->l_max_dr + 1) + 2]*0.5/r_dr);
      }
    }
    ...
    if (ppt->gauge == synchronous) {
      ...
      /* DRMD Specific Corrections */
      if (pba->has_idr_drmd == _TRUE_) {
        delta_idr_drmd -= 4.*pvecback[background_module_->index_bg_H_] * pvecback[background_module_->index_bg_a_]*alpha;
      ...
      /* DRMD Specific Corrections */
      if (pba->has_idm_drmd == _TRUE_) {
        delta_idm_drmd -= 3.*pvecback[background_module_->index_bg_H_]*pvecback[background_module_->index_bg_a_]*alpha;
    ...
    if (pba->has_dr == _TRUE_) {
      for (int n_dr = 0; n_dr < pba->N_decay_dr; n_dr++) {
        class_store_double(dataptr, delta_dr[n_dr], pba->has_dr, storeidx); // first index is from DCDM, if applicable
        class_store_double(dataptr, theta_dr[n_dr], pba->has_dr, storeidx);
        class_store_double(dataptr, shear_dr[n_dr], pba->has_dr, storeidx);
      }
      for (int n_dr = 0; n_dr < pba->N_decay_dr; n_dr++) {
        for (int l = 0; l <= ppr->l_max_dr; l++) {
          class_store_double(dataptr, y[ppw->pv->index_pt_F0_dr_species + n_dr*(ppw->pv->l_max_dr + 1) + l], pba->has_dr, storeidx);
        }
      }
    }
    ...
    if (evolve_tensor_ncdm_ == _TRUE_) {
      ...
      for(int n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
        ...
        for (int index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q ++) {
          ...
          rho_delta_ncdm += q2*epsilon*ncdm_->w_ncdm_[n_ncdm][index_q]*y[idx];
          rho_plus_p_theta_ncdm += q2*q*ncdm_->w_ncdm_[n_ncdm][index_q]*y[idx + 1];
          rho_plus_p_shear_ncdm += q2*q2/epsilon*ncdm_->w_ncdm_[n_ncdm][index_q]*y[idx + 2];
          delta_p_ncdm += q2*q2/epsilon*ncdm_->w_ncdm_[n_ncdm][index_q]*y[idx];
        }
...
int PerturbationsModule::perturb_derivs_member(double tau, double* y, double* dy, void* parameters_and_workspace, ErrorMsg error_message ) {
  ...
  if (_scalars_) {
    ...
    double delta_p_b_over_rho_b = cb2*delta_b; /* for baryons, (delta p)/rho with Ma & Bertschinger approximation: sound speed = adiabatic sound speed */
    ...
    if (pba->has_dr == _TRUE_) {
      ...
      if (pba->has_ncdm_decay_dr) {
        for (const auto& id_and_properties : ncdm_->decay_dr_map_) {
          ...
          double M_ncdm = ncdm_->M_ncdm_[ncdm_id];
          ...
          double rprime_dr = pow(a,5)/pba->H0/pba->H0*M_ncdm*Gamma*pvecback[background_module_->index_bg_number_ncdm1_ + ncdm_id];
          int q_size = ncdm_->q_size_ncdm_[ncdm_id];
          ...
          auto ComputeFl = [&](int index_q, int lmax, std::vector<double>& output) {
            double q = ncdm_->q_ncdm_[ncdm_id][index_q];
          ...
          auto compute_collision_integral = [&](int l) {
            ...
            if (ppw->approx[ppw->index_ap_ncdmfa] == (int) ncdmfa_off) {
              bool must_rescale = false;
              for (int index_q = 0; index_q < ncdm_->q_size_ncdm_[ncdm_id]; ++index_q) {
                // Compute collision term integral contribution at q
                double dq = dncdm_properties.dq[index_q];
                double w0 = dq*exp(pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + dncdm_properties.q_offset + index_q]);
                double q = ncdm_->q_ncdm_[ncdm_id][index_q];
                ...
                int psi_ind = ppw->pv->index_ncdm_[ncdm_id][index_q] + l;
              ...
              if (must_rescale) {
                ...
                double lnN = ncdm_->GetRescalingFactor(ncdm_id, ppw->pvecback + background_module_->index_bg_lnf_ncdm_decay_dr1_);
                for (int index_q = 0; index_q < ncdm_->q_size_ncdm_[ncdm_id]; ++index_q) {
                  double dq = dncdm_properties.dq[index_q];
              ...
              if (l == 0) {
                return rprime_dr*y[ppw->pv->index_ncdm_[ncdm_id][0]];
              }
              else if (l == 1) {
                return rprime_dr*y[ppw->pv->index_ncdm_[ncdm_id][0] + 1]/k;
              }
              else {
                return 0.;
              }
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) {
        ...
        for (int n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {

          /** - -----> define intermediate quantitites */

          double rho_ncdm_bg = pvecback[background_module_->index_bg_rho_ncdm1_ + n_ncdm]; /* background density */
          double p_ncdm_bg = pvecback[background_module_->index_bg_p_ncdm1_ + n_ncdm]; /* background pressure */
          double pseudo_p_ncdm = pvecback[background_module_->index_bg_pseudo_p_ncdm1_ + n_ncdm]; /* pseudo-pressure (see CLASS IV paper) */
          double w_ncdm, ca2_ncdm;
          ...
          switch (ncdm_->ncdm_types_[n_ncdm]) {
            ...
            case NCDMType::decay_dr:
              std::tie(w_ncdm, pseudo_p_over_p) = ncdm_->GetRescaledParameters(n_ncdm, a, ppw->pvecback + background_module_->index_bg_lnf_ncdm_decay_dr1_);
              ca2_ncdm = w_ncdm/3.0/(1.0+w_ncdm)*(5.0-pseudo_p_over_p); /* adiabatic sound speed */
              break;
          }
          ...
          double ceff2_ncdm = 0., cvis2_ncdm = 0.;
          ...
          const int idx = ppw->pv->index_ncdm_[n_ncdm][0]; // Check this!
          ...
          if (ncdm_->ncdm_types_[n_ncdm] == NCDMType::decay_dr) {
            // Fluid approximation correction for the decaying species; assume nonrelativistic parent
            const DecayDRProperties& dncdm_properties = ncdm_->decay_dr_map_[n_ncdm];
            double r_dr = pvecback[background_module_->index_bg_rho_dr_species_ + dncdm_properties.dr_id]*pow(a,4)/pba->H0/pba->H0;
            double rprime_dr = r_dr*a*ncdm_->M_ncdm_[n_ncdm]*dncdm_properties.Gamma*pvecback[background_module_->index_bg_number_ncdm1_ +  n_ncdm]/pvecback[background_module_->index_bg_rho_dr_species_ + dncdm_properties.dr_id];
      ...
      /** - ----> second case: use exact equation (Boltzmann hierarchy on momentum grid) */
        ...
        for (int n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
          ...
          for (int index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
            ...
            switch (ncdm_->ncdm_types_[n_ncdm]) {
              ...
              case NCDMType::decay_dr: {
                // If the current species can decay, make sure to get the time-dependent distribution function
                dlnf0_dlnq = pvecback[background_module_->index_bg_dlnfdlnq_ncdm_decay_dr1_ + ncdm_->decay_dr_map_[n_ncdm].q_offset + index_q];
                break;
              }
            }
            ...
            double epsilon = sqrt(q*q + a2*ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]);
            double qk_div_epsilon = k*q/epsilon;
    ...
    if (evolve_tensor_ncdm_ == _TRUE_) {
      ...
      for (int n_ncdm = 0; n_ncdm<pv->N_ncdm; n_ncdm++) {
        ...
        for (int index_q = 0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
          ...
          const int ncdm_idx = ppw->pv->index_ncdm_[n_ncdm][index_q];
          ...
          dy[ncdm_idx] = -qk_div_epsilon*y[ncdm_idx+1]-0.25*_SQRT6_*y[pv->index_pt_gwdot]*dlnf0_dlnq;

          /** - ----> ncdm l>0 for given momentum bin */

          for(l=1; l<pv->l_max_ncdm[n_ncdm]; l++){
            dy[ncdm_idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[ncdm_idx+(l-1)]-(l+1.)*s_l[l+1]*y[ncdm_idx+(l+1)]);
          }

          /** - ----> ncdm lmax for given momentum bin (truncation as in Ma and Bertschinger)
              but with curvature taken into account a la arXiv:1305.3261 */

          dy[ncdm_idx+l] = qk_div_epsilon*y[ncdm_idx+l-1]-(1.+l)*k*cotKgen*y[ncdm_idx+l];
        }
      }
    }
...
int PerturbationsModule::perturb_tca_slip_and_shear(double* y, void* parameters_and_workspace, ErrorMsg error_message ) {
  ...
  double a = pvecback[background_module_->index_bg_a_];
  double a_prime_over_a = pvecback[background_module_->index_bg_H_]*a;
  double a_primeprime_over_a = pvecback[background_module_->index_bg_H_prime_]*a + 2.*a_prime_over_a*a_prime_over_a;
  double R = 4./3.*pvecback[background_module_->index_bg_rho_g_]/pvecback[background_module_->index_bg_rho_b_];
  double s2_squared = 1.-3.*pba->K/k2;
  ...
  double dtau_c = -pvecthermo[thermodynamics_module_->index_th_ddkappa_]*tau_c*tau_c; /* its first derivative wrt conformal time */
  ...
  double theta_prime = (-a_prime_over_a*theta_b+k2*(cb2*delta_b+R/4.*delta_g))/(1.+R) + metric_euler;

  /** - ---> intermediate quantities for 2nd order tca: shear_g_prime at first order in tight-coupling */
  double shear_g_prime = 16./45.*(tau_c*(theta_prime+metric_shear_prime)+dtau_c*(theta_g+metric_shear));
  ...
  if (ppr->tight_coupling_approximation == (int)second_order_CLASS) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      double theta_prime_prime = ((R-1.)*a_prime_over_a*theta_prime-(a_primeprime_over_a-a_prime_over_a*a_prime_over_a)*theta_b
      ...
      double g0_prime = -a_prime_over_a*theta_prime - (a_primeprime_over_a - a_prime_over_a*a_prime_over_a)*theta_b
        + k2*(pvecthermo[thermodynamics_module_->index_th_dcb2_]*delta_b + (1./3. - cb2)*(theta_b + 0.5*pvecmetric[ppw->index_mt_h_prime]));
      double g0_prime_prime = -a_prime_over_a*theta_prime_prime-2.*(a_primeprime_over_a-a_prime_over_a*a_prime_over_a)*theta_prime
...
std::tuple<double, double, double> PerturbationsModule::RescaledNCDMPerturbations(int n_ncdm, double a, double k, perturb_workspace* ppw) {
  // Reintegrate and rescale ratios of integrated quantities in case exp(lnf) is below precision
  double rho_scaled = 0.;
  ...
  const double lnN = ncdm_->GetRescalingFactor(n_ncdm, ppw->pvecback + background_module_->index_bg_lnf_ncdm_decay_dr1_);
  
  for (int index_q = 0; index_q < ncdm_->q_size_ncdm_[n_ncdm]; index_q++) {
    const int index_pt = ppw->pv->index_ncdm_[n_ncdm][index_q];
    double dq = ncdm_->decay_dr_map_[n_ncdm].dq[index_q];
    double lnf = ppw->pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + ncdm_->decay_dr_map_[n_ncdm].q_offset + index_q];
