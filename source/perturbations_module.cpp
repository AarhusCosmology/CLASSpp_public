...
void PerturbationsModule::perturb_sources_at_tau(
...
void PerturbationsModule::perturb_output_firstline_and_ic_suffix(int index_ic,
                                                                 std::string& first_line,
                                                                 std::string& ic_suffix) const {
  first_line.clear();
  ic_suffix.clear();

  if ((ppt->has_ad) && (index_ic == index_ic_ad_)) {
    ic_suffix  = "ad";
    first_line = "for adiabatic (AD) mode (normalized to initial curvature=1) ";
  }

  if ((ppt->has_bi) && (index_ic == index_ic_bi_)) {
    ic_suffix  = "bi";
    first_line = "for baryon isocurvature (BI) mode (normalized to initial entropy=1)";
  }

  if ((ppt->has_cdi) && (index_ic == index_ic_cdi_)) {
    ic_suffix  = "cdi";
...
void PerturbationsModule::perturb_init() {
  /** Summary: */
  ...
  if (all_species_.count("DCDM_DR")) {
    class_test((ppt->has_cdi) || (ppt->has_bi) || (ppt->has_nid) || (ppt->has_niv),
               "Non-adiabatic initial conditions not coded in presence of decaying dark matter");
  }

  bool has_dncdm_dr = false;
  ...
  class_test(ppt->has_vectors, "Vectors not coded yet");

  if ((ppt->has_niv) && (ppt->perturbations_verbose > 0)) {
    printf(
  ...
  if (ppt->z_max_pk > thermodynamics_module_->z_rec_) {
    ...
    class_test(has_source_delta_m_,
               "You requested a very high z_pk=%e, higher than z_rec=%e. This works very well when "
...
void PerturbationsModule::perturb_indices_of_perturbs() {
  /** Summary: */
  ...
  for (index_md = 0; index_md < md_size_; index_md++) {
    ...
    if (_scalars_) {
      ...
      if ((ppt->has_cl_cmb_lensing_potential) || (ppt->has_cl_lensing_potential)) {
        has_lss_                 = true;
        has_source_phi_plus_psi_ = true;
      }

      if ((ppt->has_pk_matter) || (ppt->has_nl_corrections_based_on_delta_m)) {
        has_lss_            = true;
...
void PerturbationsModule::perturb_timesampling_for_sources() {
  ...
  if ((has_cmb_) || (ppt->has_perturbed_recombination)) {
...
void PerturbationsModule::perturb_workspace_init(int index_md, perturb_workspace* ppw) {
  /** Summary: */
  ...
  ppw->s_l.resize(ppw->max_l_max + 1);
...
void PerturbationsModule::perturb_solve(int index_md,
                                        int index_ic,
                                        int index_k,
...
void PerturbationsModule::perturb_prepare_k_output() {
...
void PerturbationsModule::perturb_find_approximation_number(
    int index_md,
    double k,
    perturb_workspace* ppw,
    double tau_ini,
...
void PerturbationsModule::perturb_find_approximation_switches(
    int index_md,
    double k,
    perturb_workspace* ppw,
    double tau_ini,
...
void PerturbationsModule::perturb_vector_init(
    int index_md,
    int index_ic,
    double k,
    double tau,
  ...
  if (_tensors_) {
    ...
    if (evolve_tensor_ur_) {
      ...
      ur_lay.idx_theta = index_pt++;
      ur_lay.idx_shear = index_pt++;
      if (ur_lay.l_max >= 3) {
  ...
  if (_scalars_) {
    for (size_t i = 0; i < all_species_.size(); ++i) {
      all_species_[i]->MarkUsedInSources(*ppv->species_layouts[i],
                                         ppw,
  ...
  if (_tensors_) {
    for (size_t i = 0; i < all_species_.size(); ++i) {
      all_species_[i]->MarkTensorUsedInSources(*ppv->species_layouts[i],
                                               ppw,
                                               ppv->used_in_sources.data());
    }
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if ((pa_old[ppw->index_ap_tca] == (int) tca_on) &&
        ...
        ppv->y[new_g_lay.idx_shear] = ppw->tca_shear_g;

        ppv->y[new_g_lay.idx_l3] =
            6. / 7. * k / ppw->pvecthermo[thermodynamics_module_->index_th_dkappa_] * ppw->s_l[3] *
            ppv->y[new_g_lay.idx_shear]; /* second-order tight-coupling approximation for l=3 */
        ppv->y[new_g_lay.idx_pol0] =
            2.5 *
            ppv->y
                [new_g_lay
                     .idx_shear]; /* first-order tight-coupling approximation for polarization, l=0 */
        ppv->y[new_g_lay.idx_pol1] =
            k / ppw->pvecthermo[thermodynamics_module_->index_th_dkappa_] *
            (5. - 2. * ppw->s_l[2]) / 6. *
            ppv->y
                [new_g_lay
                     .idx_shear]; /* second-order tight-coupling approximation for polarization, l=1 */
        ppv->y[new_g_lay.idx_pol2] =
            0.5 *
            ppv->y
                [new_g_lay
                     .idx_shear]; /* first-order tight-coupling approximation for polarization, l=2 */
        ppv->y[new_g_lay.idx_pol3] =
            k / ppw->pvecthermo[thermodynamics_module_->index_th_dkappa_] * 3. * ppw->s_l[3] / 14. *
            ppv->y
                [new_g_lay
                     .idx_shear]; /* second-order tight-coupling approximation for polarization, l=3 */

        /* perturbed recombination */
        /* the initial conditions are set when tca is switched off (current block) */
        if (ppt->has_perturbed_recombination) {
          const size_t b_pr_i  = all_species_.index_of("Baryons");
          const auto& b_pr_lay = static_cast<const BaryonsSpecies::PerturbLayout&>(
              *ppv->species_layouts[b_pr_i]);
          ppv->y[ppv->index_pt_perturbed_recombination_delta_temp] = 1. / 3. *
                                                                     ppv->y[b_pr_lay.idx_delta];
      ...
      if ((ppt->has_perturbed_recombination) && (pa_old[ppw->index_ap_tca] == (int) tca_off)) {
   ...
   of variables */
      ...
      if (all_species_.count("IDM_DRMD_IDR_DRMD")) {
        ...
        if ((pa_old[ppw->index_ap_tca_idm_drmd] == (int) tca_idm_drmd_on) &&
          ...
          ppv->y[idm_drm_new_lay.idx_delta] = ppw->pv->y[idm_drm_old_lay.idx_delta];

          ppv->y[drm_new_lay.idx_delta] = delta_idr;

          ppv->y[drm_new_lay.idx_theta] = ppw->pv->y[drm_old_lay.idx_theta];

          ppv->y[idm_drm_new_lay.idx_theta] =
    ...
    if (_vectors_) {
      ...
      {
        const size_t b_vsw_i  = all_species_.index_of("Baryons");
        const auto& b_vsw_old = static_cast<const BaryonsSpecies::PerturbLayout&>(
            *ppw->pv->species_layouts[b_vsw_i]);
        auto& b_vsw_new = static_cast<BaryonsSpecies::PerturbLayout&>(
            *ppv->species_layouts[b_vsw_i]);
        ppv->y[b_vsw_new.idx_theta] = ppw->pv->y[b_vsw_old.idx_theta];
      }
    ...
    if (_tensors_) {
      ...
      if (evolve_tensor_ur_) {
        ...
        ppv->y[new_ur_lay.idx_delta] = ppw->pv->y[old_ur_lay.idx_delta];
        ppv->y[new_ur_lay.idx_theta] = ppw->pv->y[old_ur_lay.idx_theta];
        ppv->y[new_ur_lay.idx_shear] = ppw->pv->y[old_ur_lay.idx_shear];
...
void PerturbationsModule::perturb_initial_conditions(
  ...
  if (_scalars_) {
    ...
    double rho_r  = all_species_.photons().Rho(ppw->pvecback.data());
    double rho_m  = all_species_.baryons().Rho(ppw->pvecback.data());
    double rho_nu = 0.;
    ...
    for (auto& [name, sp] : all_species_) {
      ...
      switch (sp->energy_type()) {
        ...
        case BaseSpecies::EnergyType::Other:
          ...
          rho_r += 3. * sp->P(ppw->pvecback.data());
          rho_m += rho - 3. * sp->P(ppw->pvecback.data());
          break;
        default:  // DarkEnergy: no rho_r/rho_m contribution
          break;
      }

      rho_nu += sp->FreestreamingRho(ppw->pvecback.data());
    }
    ...
    double fracg = all_species_.photons().Rho(ppw->pvecback.data()) / rho_r;

    /* f_b = Omega_b(t_i) / Omega_m(t_i) */
    double fracb = all_species_.baryons().Rho(ppw->pvecback.data()) / rho_m;

    /* f_cdm = Omega_cdm(t_i) / Omega_m(t_i) */
    double fraccdm = 0.;
    if (all_species_.count("CDM"))
      fraccdm = all_species_.at("CDM")->Rho(ppw->pvecback.data()) / rho_m;
    ...
    if (all_species_.count("IDM_DRMD_IDR_DRMD")) {
      ...
      if (drmd.idm_drmd().IsPresent())
        fracidm_drmd = drmd.idm_drmd().Rho(ppw->pvecback.data()) / rho_m;
    }
    ...
    if (ppt->gauge == possible_gauges::newtonian) {
      ...
      */
      ...
      ic_ctx.alpha_prime = 0.;  // no species IC transform currently requires alpha'
      {
        size_t sp_idx = 0;
        for (auto& sp : all_species_) {
          sp->PerturbSynchronousToNewtonian(*ppw->pv->species_layouts[sp_idx],
                                            ppw->pv->y.data(),
                                            ic_ctx);
          ++sp_idx;
        }
...
void PerturbationsModule::perturb_approximations(int index_md,
                                                 double k,
                                                 double tau,
                                                 perturb_workspace* ppw) {
  /** Summary: */
...
void PerturbationsModule::perturb_timescale_member(double tau,
  ...
  ppw        = pppaw->ppw;
  pvecback   = ppw->pvecback.data();
  pvecthermo = ppw->pvecthermo.data();
...
void PerturbationsModule::perturb_einstein(
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == possible_gauges::newtonian) {
      ...
      if (ppw->approx[ppw->index_ap_rsa] == (int) rsa_on) {
        perturb_rsa_delta_and_theta(k, y, a_prime_over_a, ppw->pvecthermo.data(), ppw);
      }

      if (resolved_.idm_dr_idr && (ppw->approx[ppw->index_ap_rsa_idr] == (int) rsa_idr_on)) {
    ...
    if (ppt->gauge == possible_gauges::synchronous) {
      ...
      if (resolved_.idm_dr_idr && (ppw->approx[ppw->index_ap_rsa_idr] == (int) rsa_idr_on)) {
        ...
        ppw->rho_plus_p_theta += 4. / 3. * resolved_.idm_dr_idr->idr().Rho(ppw->pvecback.data()) *
                                 ppw->rsa_theta_idr;
      }
      ...
      if (ppw->approx[ppw->index_ap_tca] == (int) tca_on) {
        ...
        ppw->rho_plus_p_shear += 4. / 3. * all_species_.photons().Rho(ppw->pvecback.data()) *
                                 shear_g;
      }
      ...
      if (resolved_.idm_dr_idr && (ppw->approx[ppw->index_ap_tca_idm_dr] == (int) tca_idm_dr_on)) {
        ...
        ppw->rho_plus_p_shear += 4. / 3. * resolved_.idm_dr_idr->idr().Rho(ppw->pvecback.data()) *
                                 shear_idr;
      }
...
void PerturbationsModule::perturb_total_stress_energy(int index_md,
                                                      double k,
                                                      double* y,
                                                      perturb_workspace* ppw) {
  /** Summary: */
    ...
    /** - --> photon contribution to vector sources: */
    {
      ...
      if (ppw->approx[ppw->index_ap_rsa] ==
        ...
        if (ppw->approx[ppw->index_ap_tca] ==
          ...
          ppw->vector_source_v += 4. / 3. * a2 * all_species_.photons().Rho(ppw->pvecback.data()) *
                                  (-1. / 4. * _SQRT2_) *
                                  (y[g_vec_lay.idx_delta] + 2. * y[g_vec_lay.idx_delta] +
                                   y[g_vec_lay.idx_shear]);

          ppw->vector_source_pi += 1. / 3. * a2 * all_species_.photons().Rho(ppw->pvecback.data()) *
                                   (6. * _SQRT2_ / 5. / sqrt(1. - 2. * pba->K / k / k)) *
                                   (4. / 3. / k * y[g_vec_lay.idx_theta] + y[g_vec_lay.idx_l3]);
        }
      }
    }
  ...
  if (_tensors_) {
    ...
    if (evolve_tensor_ur_) {
      ...
      const auto& ur_tens_lay  = ppw->pv->tensor_ur_layout;
      ppw->gw_source          += (-_SQRT6_ * 4 * a2 * rho_relativistic *
                                  (1. / 15. * y[ur_tens_lay.idx_delta] +
                                   4. / 21. * y[ur_tens_lay.idx_shear] +
                                   1. / 35. * y[ur_tens_lay.idx_l3 + 1]));
    }
...
void PerturbationsModule::perturb_sources_member(
  ...
  double* pvecthermo = ppw->pvecthermo.data();
  double* pvecmetric = ppw->pvecmetric.data();
  ...
  if (_scalars_) {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int) rsa_on) {
      ...
      delta_g = y[g_src_lay.idx_delta];
      if (ppw->approx[ppw->index_ap_tca] == (int) tca_on)
        P = 5. * ppw->s_l[2] * ppw->tca_shear_g / 8.; /* (2.5+0.5+2)shear_g/8 */
      else
        P = (y[g_src_lay.idx_pol0] + y[g_src_lay.idx_pol2] +
             2. * ppw->s_l[2] * y[g_src_lay.idx_shear]) /
            8.;
    }
    ...
    if (has_source_t_) {
      ...
      /*
        ...
        _set_source_(index_tp_t1_) = pvecthermo[thermodynamics_module_->index_th_exp_m_kappa_] * k* pvecmetric[ppw->index_mt_psi] + pvecthermo[thermodynamics_module_->index_th_g_] * y[b_src_lay.idx_theta]/k;
      ...
      */
      ...
      if (ppt->gauge == possible_gauges::newtonian) {
        _set_source_(index_tp_t0_) =
            ...
            ppt->switch_dop / k / k *
                (pvecthermo[thermodynamics_module_->index_th_g_] * dy[b_src_lay.idx_theta] +
                 pvecthermo[thermodynamics_module_->index_th_dg_] * y[b_src_lay.idx_theta]);
      ...
      /*
        ...
        _set_source_(index_tp_t1_) = pvecthermo[thermodynamics_module_->index_th_g_] * y[b_src_lay.idx_theta] / k;
        ...
        _set_source_(index_tp_t0_) =
            ...
            ppt->switch_dop *
                (pvecthermo[thermodynamics_module_->index_th_g_] *
                     (dy[b_src_lay.idx_theta] / k / k + pvecmetric[ppw->index_mt_alpha_prime]) +
                 pvecthermo[thermodynamics_module_->index_th_dg_] *
                     (y[b_src_lay.idx_theta] / k / k + pvecmetric[ppw->index_mt_alpha]));
  ...
  if (_tensors_) {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int) rsa_off) {
      ...
      if (ppw->approx[ppw->index_ap_tca] == (int) tca_off) {
        ...
        l4_g    = y[g_pv_lay.idx_delta + 4];
        pol0_g  = y[g_pv_lay.idx_pol0];
        pol2_g  = y[g_pv_lay.idx_pol2];
    ...
    if (evolve_tensor_ncdm_) {
      ...
      for (size_t i = 0; i < all_species_.size(); ++i) {
        ...
        const double rho        = ncdm_sp->Rho(ppw->pvecback.data());
        const double rho_plus_p = rho + ncdm_sp->P(ppw->pvecback.data());
        const double delta_ncdm = rho_delta_ncdm / rho;
...
void PerturbationsModule::perturb_derivs_member(double tau,
  ...
  if (_vectors_) {
    ...
    if (ppt->gauge == possible_gauges::synchronous) {
      ...
      dy[b_vec_dr_lay.idx_theta] = -(1 - 3. * cb2) * a_prime_over_a * y[b_vec_dr_lay.idx_theta] -
                                   _SQRT2_ / 4. *
                                       pvecthermo[thermodynamics_module_->index_th_dkappa_] *
                                       (delta_g_vec + 2. * _SQRT2_ * y[b_vec_dr_lay.idx_theta]) +
  ...
  if (_tensors_) {
    ...
    if (evolve_tensor_ur_) {
      ...
      const int idx_shear = ur_lay.idx_shear;
      const int idx_l3    = ur_lay.idx_l3;
      const int l_max     = ur_lay.l_max;

      dy[idx_delta] = -4. / 3. * y[idx_theta] + _SQRT6_ * y[pv->index_pt_gwdot];

      dy[idx_theta] = k2 * (y[idx_delta] / 4. - s2_squared * y[idx_shear]);

      dy[idx_shear] = (4. / 15. * y[idx_theta] - 3. / 10. * k * s_l[3] / s_l[2] * y[idx_shear + 1]);
...
void PerturbationsModule::perturb_tca_slip_and_shear(double* y, void* parameters_and_workspace) {
  /** Summary: */
  ...
  if (ppw->approx[ppw->index_ap_rsa] == (int) rsa_off) {
    ...
    theta_g = y[g_tca_lay.idx_theta];
  }
  double delta_b = y[b_tca_lay.idx_delta];
  double theta_b = y[b_tca_lay.idx_theta];
  double cb2     = pvecthermo[thermodynamics_module_->index_th_cb2_];
...
void PerturbationsModule::perturb_rsa_delta_and_theta(
    ...
    if (ppr->radiation_streaming_approximation == static_cast<int>(rsa_method::rsa_MD_with_reio)) {
      ...
      ppw->rsa_theta_g += 3. / k2 *
                          (ppw->pvecthermo[thermodynamics_module_->index_th_ddkappa_] *
                               y[b_rsa_lay.idx_theta] +
                           ppw->pvecthermo[thermodynamics_module_->index_th_dkappa_] *
                               (-a_prime_over_a * y[b_rsa_lay.idx_theta] +
                                ppw->pvecthermo[thermodynamics_module_->index_th_cb2_] * k2 *
                                    y[b_rsa_lay.idx_delta] +
  ...
  ppw->delta_rho += all_species_.photons().Rho(ppw->pvecback.data()) * ppw->rsa_delta_g;
  ppw->delta_p   += 1. / 3. * all_species_.photons().Rho(ppw->pvecback.data()) * ppw->rsa_delta_g;
...
void PerturbationsModule::perturb_rsa_idr_delta_and_theta(
    double k, double* y, double a_prime_over_a, double* pvecthermo, perturb_workspace* ppw) {
  /* - define local variables */
