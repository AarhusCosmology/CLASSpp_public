              ...
              if (index_ic1 == index_ic2) {
                ...
                */
                ...
                if (pk > sqrt(pk1 * pk2))
                  ...
                  lnpk_[index_md][index_k * ic_ic_size_[index_md] + index_ic1_ic2] = pk / sqrt(pk1 *
                                                                                               pk2);
              }
            }
            else {
      ...
      for (index_ic2 = index_ic1 + 1; index_ic2 < ic_size_[index_md]; index_ic2++) {
        if (_scalarsEXT_) {
          ...
          if ((ppt->has_ad == _TRUE_) && (ppt->has_niv == _TRUE_) &&
              (((index_ic1 == perturbations_module_->index_ic_ad_) &&
               ...
               ((index_ic2 == perturbations_module_->index_ic_ad_) &&
                (index_ic1 == perturbations_module_->index_ic_niv_)))) {
        ...
        if (one_correlation == 0.) {
          ...
          tilt_[index_md][index_ic1_ic2] =
              0.5 * (tilt_[index_md][index_ic1_ic1] + tilt_[index_md][index_ic2_ic2]) + one_tilt;
          running_[index_md][index_ic1_ic2] = 0.5 * (running_[index_md][index_ic1_ic1] +
...
int PrimordialModule::primordial_inflation_potential(double phi,
  ...
  switch (ppm->potential) {
    ...
    case polynomial:
      ...
      *V   = ppm->V0 + phi * ppm->V1 + pow(phi, 2) / 2. * ppm->V2 + pow(phi, 3) / 6. * ppm->V3 +
             pow(phi, 4) / 24. * ppm->V4;
    ...
    case higgs_inflation:
      ...
      *dV  = ppm->V0 / 4. / pow(8. * _PI_, 2) / ppm->V1 / ppm->V1 *
             (dp * pow(mu, 4) + 4. * p * dmu * pow(mu, 3));
      *ddV = ppm->V0 / 4. / pow(8. * _PI_, 2) / ppm->V1 / ppm->V1 *
             (ddp * pow(mu, 4) + 8. * dp * dmu * pow(mu, 3) + 4. * p * ddmu * pow(mu, 3) +
              12. * p * pow(dmu * mu, 2));
...
int PrimordialModule::primordial_inflation_one_k(
  ...
  } while ((k / aH >= ppr->primordial_inflation_ratio_max) ||
...
int PrimordialModule::primordial_inflation_evolve_background(double* y,
  ...
  if ((direction == backward) && ((ppm->primordial_spec_type == inflation_V) ||
                                  (ppm->primordial_spec_type == inflation_V_end))) {
  ...
  switch (target) {
    ...
    case _end_inflation_:
      ...
      quantity = -pow(dy[index_in_a_] / y[index_in_a_], 2) +
                 4 * _PI_ * y[index_in_dphi_] * y[index_in_dphi_];
      if (time == conformal)
        quantity /= pow(y[index_in_a_], 2);
  ...
  switch (target) {
    case _aH_:
      switch (time) {
        case proper:
          ...
          break;
        case conformal:
          dtau = (stop / (dy[index_in_a_] / y[index_in_a_]) - 1.) /
                 (dy[index_in_a_] / y[index_in_a_]);
          break;
      }
      break;
    case _phi_:
    ...
    case _end_inflation_:
      ...
      switch (time) {
        case proper:
          dtau = -quantity /
                 (8. * _PI_ * dy[index_in_phi_] *
                  (dy[index_in_a_] * dy[index_in_phi_] + y[index_in_a_] * dy[index_in_dphi_]));

          break;
        case conformal:
          dtau = -quantity / (8. * _PI_ / y[index_in_a_] / y[index_in_a_] * dy[index_in_phi_] *
                              dy[index_in_dphi_]);
          break;
      }
      break;
  ...
  if (epsilon > 1.) {
    ...
    do {
      ...
      if (epsilon < ppr->primordial_inflation_small_epsilon)
        ...
        phi_right = phi_mid;
    } while (fabs(epsilon - ppr->primordial_inflation_small_epsilon) >
             ppr->primordial_inflation_small_epsilon_tol);
    ...
    switch (ppm->phi_pivot_method) {
      case ln_aH_ratio_auto:
        ...
        Omega_g0 = (4. * sigma_B / _c_ * pow(2.726, 4.)) /
                   (3. * _c_ * _c_ * 1.e10 * h * h / _Mpc_over_m_ / _Mpc_over_m_ / 8. / _PI_ / _G_);
        Omega_r0 = 3.046 * 7. / 8. * pow(4. / 11., 4. / 3.) * Omega_g0;

        target = log(H0 / 0.05 * pow(Omega_r0, 0.5) * pow(2. / 100., 1. / 12.) *
                     pow(rho_end / rho_c0, 0.25));
    ...
    class_test(log(ratio_try) < target,
               ...
               "must increase precision parameter primordial_inflation_extra_efolds",
               log(ratio_try),
               target);
...
int PrimordialModule::primordial_inflation_derivs_member(double tau,
  ...
  switch (ppm->primordial_spec_type) {
        ...
        case forward:
          ...
          switch (ppipaw->time) {
            case conformal:
              ...
              // a H = a'/a
              ppipaw->aH = sqrt((8 * _PI_ / 3.) * (0.5 * y[index_in_dphi_] * y[index_in_dphi_] +
                                                   ppipaw->a2 * ppipaw->V));
              // 1: a
            ...
            case proper:
              ...
              // a H = adot
              ppipaw->aH = y[index_in_a_] *
                           sqrt((8 * _PI_ / 3.) *
                                (0.5 * y[index_in_dphi_] * y[index_in_dphi_] + ppipaw->V));
              // 1: a
              dy[index_in_a_] = ppipaw->aH;
              // 2: phi
              dy[index_in_phi_] = y[index_in_dphi_];
              // 3: dphi/dt
              dy[index_in_dphi_] = -3. * ppipaw->aH / y[index_in_a_] * y[index_in_dphi_] -
                                   ppipaw->dV;
              break;
          }

          // z''/z (assumes that conformal time is requested)
          ppipaw->zpp_over_z = 2 * ppipaw->aH * ppipaw->aH - ppipaw->a2 * ppipaw->ddV -
                               4. * _PI_ *
                                   (7. * y[index_in_dphi_] * y[index_in_dphi_] +
          ...
          ppipaw->app_over_a = 2. * ppipaw->aH * ppipaw->aH -
                               4. * _PI_ * y[index_in_dphi_] * y[index_in_dphi_];
    ...
    case inflation_H:
      ...
      // z''/z (assumes that conformal time is requested)
      ppipaw->zpp_over_z = 2. * ppipaw->a2 * ppipaw->H * ppipaw->H -
                           ...
                           1. / 4. / _PI_ / _PI_ * ppipaw->a2 * ppipaw->dH * ppipaw->dH *
                               ppipaw->ddH / ppipaw->H +
                           1. / 2. / _PI_ * ppipaw->a2 * ppipaw->dH * ppipaw->dH +
                           1. / 8. / _PI_ / _PI_ * ppipaw->a2 * ppipaw->dH * ppipaw->dH *
                               ppipaw->dH * ppipaw->dH / ppipaw->H / ppipaw->H;

      // a''/a (assumes that conformal time is requested)
      ppipaw->app_over_a = 2. * ppipaw->a2 * ppipaw->H * ppipaw->H -
                           4. * _PI_ * dy[index_in_phi_] * dy[index_in_phi_];
