...
public:
  ...
  std::vector<std::vector<double>> nl_corr_density_;   /**< nl_corr_density[index_pk][index_tau * perturbations_module_->k_size_ + index_k] */

  std::vector<short> is_non_zero_;  /**< for a given mode, is_non_zero[index_md][index_ic1_ic2] is set to true if the pair of initial conditions (index_ic1, index_ic2) are statistically correlated, or to false if they are uncorrelated */
...
private:
  ...
  int nonlinear_get_source(int index_k, int index_ic, int index_tp, int index_tau, const std::vector<std::vector<double>>& sources, double* source);
  ...
  int nonlinear_hmcode(int index_pk, int index_tau, double tau, double*pk_nl, std::vector<std::vector<double>>& lnpk_l, std::vector<std::vector<double>>& ddlnpk_l, double* k_nl, short* halofit_found_k_max, nonlinear_workspace* pnw);
  ...
  std::vector<std::vector<double>> ddln_pk_ic_l_; /**< second derivative of above array with respect to log(tau), for spline interpolation. So:
  ...
  std::vector<std::vector<double>> ln_pk_l_;   /**< Total matter power spectrum summed over initial conditions (linear).
                          Only depends on indices index_pk,index_k, index_tau as:
                          ln_pk[index_pk][index_tau * k_size_ + index_k]
                       */

  std::vector<std::vector<double>> ddln_pk_l_; /**< second derivative of above array with respect to log(tau), for spline interpolation. */

  std::vector<std::vector<double>> ln_pk_nl_;   /**< Total matter power spectrum summed over initial conditions (nonlinear).
                          Only depends on indices index_pk,index_k, index_tau as:
                          ln_pk[index_pk][index_tau * k_size_ + index_k]
                       */

  std::vector<std::vector<double>> ddln_pk_nl_; /**< second derivative of above array with respect to log(tau), for spline interpolation. */
  ...
  std::vector<std::vector<double>> k_nl_;              /**< wavenumber at which non-linear corrections become important,
