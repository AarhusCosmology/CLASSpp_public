...
struct precision
{
  ...
  */
   ...
   */
  int recfast_Heswitch = 6; /**< from recfast 1.4, specifies how accurate the Helium recombination should be handled */
  double recfast_fudge_He = 0.86; /**< from recfast 1.4, fugde factor for Peeble's equation coefficient of Helium */

  /*
   * Recfast 1.5 parameters
   */
  int recfast_Hswitch = _TRUE_; /**< from recfast 1.5, specifies how accurate the Hydrogen recombination should be handled */
  double recfast_fudge_H = 1.14; /**< from recfast 1.4, fudge factor for Peeble's equation coeffient of Hydrogen */
  double recfast_delta_fudge_H = -0.015; /**< from recfast 1.5.2, increasing Hydrogen fudge factor if Hswitch is enabled */
  double recfast_AGauss1 = -0.14; /**< from recfast 1.5, Gaussian Peeble prefactor fit, amplitude */
  double recfast_AGauss2 = 0.079; /**< from recfast 1.5.2, Gaussian Peeble prefactor fit, amplitude */
  ...
  double recfast_delta_z_He_1 = 50.0; /**< Smoothing factor for recombination approximation switching, found to be OK on 3.09.10 */
  double recfast_z_He_2 = 5000.0; /**< from recfast 1.4, Ending value of Helium recombination 1 */
  double recfast_delta_z_He_2 = 100.0; /**< Smoothing factor for recombination approximation switching, found to be OK on 3.09.10 */
  double recfast_z_He_3 = 3500.0; /**< from recfast 1.4, Starting value of Helium recombination 2 */
  double recfast_delta_z_He_3 = 50.0; /**< Smoothing factor for recombination approximation switching, found to be OK on 3.09.10 */
  double recfast_x_He0_trigger = 0.995; /**< Switch for Helium full calculation during reco, raised from 0.99 to 0.995 for smoother Helium */
  double recfast_x_He0_trigger2 = 0.995; /**< Switch for Helium full calculation during reco, for changing Helium flag, raised from 0.985 to same as previous one for smoother Helium */
  double recfast_x_He0_trigger_delta = 0.05; /**< Smoothing factor for recombination approximation switching, found to be OK on 3.09.10 */
  double recfast_x_H0_trigger = 0.995; /**< Switch for Hydrogen full calculation during reco, raised from 0.99 to 0.995 for smoother Hydrogen */
  double recfast_x_H0_trigger2 = 0.995; /**< Switch for Hydrogen full calculation during reco, for changing Hydrogen flag, raised from 0.98 to same as previous one for smoother Hydrogen */
  double recfast_x_H0_trigger_delta = 0.05; /**< Smoothing factor for recombination approximation switching, found to be OK on 3.09.10 */

  double recfast_H_frac = 1.0e-3; /**< from recfast 1.4, specifies the time at which the temperature evolution is calculated by the more precise equation */
  ...
  double reionization_optical_depth_tol = 1.0e-4; /**< Relative tolerance on finding the user-given optical depth of reionization given a certain redshift of reionization */
  double reionization_start_factor = 8.0; /**< Searching optical depth corresponding to the redshift is started from an initial offset beyond z_reionization_start, multiplied by reionization_width */

  int thermo_rate_smoothing_radius = 50; /**< Smoothing in redshift of the variation rate of \f$ \exp(-\kappa) \f$, g, and \f$ \frac{dg}{d\tau} \f$ that is used as a timescale afterwards */
  ...
  FileName hyrec_two_photon_tables_file; /**< File containing the two-photon interaction parameter of hyrec */

  double k_min_tau0 = 0.1; /**< number defining k_min for the computation of Cl's and P(k)'s (dimensionless): (k_min tau_0), usually chosen much smaller than one */

  double k_max_tau0_over_l_max = 2.4; /**< number defining k_max for the computation of Cl's (dimensionless): (k_max tau_0)/l_max, usually chosen around two */
  double k_step_sub = 0.05; /**< step in k space, in units of one period of acoustic oscillation at decoupling, for scales inside sound horizon at decoupling */
  double k_step_super = 0.002; /**< step in k space, in units of one period of acoustic oscillation at decoupling, for scales above sound horizon at decoupling */
  double k_step_transition = 0.2; /**< dimensionless number regulating the transition from 'sub' steps to 'super' steps. Decrease for more precision. */
  double k_step_super_reduction = 0.1; /**< the step k_step_super is reduced by this amount in the k-->0 limit (below scale of Hubble and/or curvature radius) */

  double k_per_decade_for_pk = 10.0; /**< if values needed between kmax inferred from k_oscillations and k_kmax_for_pk, this gives the number of k per decade outside the BAO region*/

  double idmdr_boost_k_per_decade_for_pk = 1.0; /**< boost factor for the case of DAO in idm-idr models */

  double k_per_decade_for_bao = 70.0; /**< if values needed between kmax inferred from k_oscillations and k_kmax_for_pk, this gives the number of k per decade inside the BAO region (for finer sampling)*/

  double k_bao_center = 3.0; /**< in ln(k) space, the central value of the BAO region where sampling is finer is defined as k_rec times this number (recommended: 3, i.e. finest sampling near 3rd BAO peak) */

  double k_bao_width = 4.0; /**< in ln(k) space, width of the BAO region where sampling is finer: this number gives roughly the number of BAO oscillations well resolved on both sides of the central value (recommended: 4, i.e. finest sampling from before first up to 3+4=7th peak) */

  double start_small_k_at_tau_c_over_tau_h = 0.0015; /**< largest wavelengths start being sampled when universe is sufficiently opaque. This is quantified in terms of the ratio of thermo to hubble time scales, \f$ \tau_c/\tau_H \f$. Start when start_largek_at_tau_c_over_tau_h equals this ratio. Decrease this value to start integrating the wavenumbers earlier in time. */
  ...
  double start_sources_at_tau_c_over_tau_h = 0.008; /**< sources start being sampled when universe is sufficiently opaque. This is quantified in terms of the ratio of thermo to hubble time scales, \f$ \tau_c/\tau_H \f$. Start when start_sources_at_tau_c_over_tau_h equals this ratio. Decrease this value to start sampling the sources earlier in time. */

  int tight_coupling_approximation = (int)compromise_CLASS; /**< method for tight coupling approximation */

  double idm_dr_tight_coupling_trigger_tau_c_over_tau_k = 0.01; /**< when to switch off the dark-tight-coupling approximation, first condition (see normal tca for full definition) */
  double idm_dr_tight_coupling_trigger_tau_c_over_tau_h = 0.015; /**< when to switch off the dark-tight-coupling approximation, second condition (see normal tca for full definition) */

  double idm_drmd_tight_coupling_trigger_G_over_aH = 100000; /**< when to switch off the dark-tight-coupling approximation in DRMD, should be larger than at least 100 (currently set to a very high number as the code runs perfectly fine without the approximation.) */
  int l_max_g = 12; /**< number of momenta in Boltzmann hierarchy for photon temperature (scalar), at least 4 */
  int l_max_pol_g = 10; /**< number of momenta in Boltzmann hierarchy for photon polarization (scalar), at least 4 */
  int l_max_dr = 17; /**< number of momenta in Boltzmann hierarchy for decay radiation, at least 4 */
  int l_max_dr_col = 17; /**< number of collision terms in Boltzmann hierarchy for decay radiation, at least 2 */
  int l_max_ur = 17; /**< number of momenta in Boltzmann hierarchy for relativistic neutrino/relics (scalar), at least 4 */
  int l_max_idr = 17; /**< number of momenta in Boltzmann hierarchy for interacting dark radiation */
  int l_max_ncdm = 17; /**< number of momenta in Boltzmann hierarchy for relativistic neutrino/relics (scalar), at least 4 */
  int l_max_g_ten = 5; /**< number of momenta in Boltzmann hierarchy for photon temperature (tensor), at least 4 */
  int l_max_pol_g_ten = 5; /**< number of momenta in Boltzmann hierarchy for photon polarization (tensor), at least 4 */
  ...
  int idr_streaming_approximation = rsa_idr_none; /**< method for dark radiation free-streaming approximation */
  double idr_streaming_trigger_tau_over_tau_k = 50.0; /**< when to switch on dark radiation (idr) free-streaming approximation, first condition */
  double idr_streaming_trigger_tau_c_over_tau = 10.0; /**< when to switch on dark radiation (idr) free-streaming approximation, second condition */
  ...
  int ncdm_fluid_approximation = ncdmfa_CLASS; /**< method for non-cold dark matter fluid approximation */
  ...
  double k_per_decade_primordial = 10.0; /**< logarithmic sampling for primordial spectra (number of points per decade in k space) */

  double primordial_inflation_ratio_min = 100.0; /**< for each k, start following wavenumber when aH = k/primordial_inflation_ratio_min */
  double primordial_inflation_ratio_max = 1.0/50.0; /**< for each k, stop following wavenumber, at the latest, when aH = k/primordial_inflation_ratio_max */
  int primordial_inflation_phi_ini_maxit = 10000; /**< maximum number of iteration when searching a suitable initial field value phi_ini (value reached when no long-enough slow-roll period before the pivot scale) */
  double primordial_inflation_pt_stepsize = 0.01; /**< controls the integration timestep for inflaton perturbations */
  double primordial_inflation_bg_stepsize = 0.005; /**< controls the integration timestep for inflaton background */
  double primordial_inflation_tol_integration = 1.0e-3; /**< controls the precision of the ODE integration during inflation */
  double primordial_inflation_attractor_precision_pivot = 0.001; /**< targeted precision when searching attractor solution near phi_pivot */
  double primordial_inflation_attractor_precision_initial = 0.1; /**< targeted precision when searching attractor solution near phi_ini */
  int primordial_inflation_attractor_maxit = 10; /**< maximum number of iteration when searching attractor solution */
  double primordial_inflation_tol_curvature = 1.0e-3; /**< for each k, stop following wavenumber, at the latest, when curvature perturbation R is stable up to to this tolerance */
  double primordial_inflation_aH_ini_target = 0.9; /**< control the step size in the search for a suitable initial field value */
  double primordial_inflation_end_dphi = 1.0e-10; /**< first bracketing width, when trying to bracket the value phi_end at which inflation ends naturally */
  double primordial_inflation_end_logstep = 10.0; /**< logarithmic step for updating the bracketing width, when trying to bracket the value phi_end at which inflation ends naturally */
  double primordial_inflation_small_epsilon = 0.1; /**< value of slow-roll parameter epsilon used to define a field value phi_end close to the end of inflation (doesn't need to be exactly at the end): epsilon(phi_end)=small_epsilon (should be smaller than one) */
  double primordial_inflation_small_epsilon_tol = 0.01; /**< tolerance in the search for phi_end */
  double primordial_inflation_extra_efolds = 2.0; /**< a small number of efolds, irrelevant at the end, used in the search for the pivot scale (backward from the end of inflation) */
  ...
  int l_linstep = 40; /**< factor for logarithmic spacing of values of l over which bessel and transfer functions are sampled */

  double l_logstep = 1.12; /**< maximum spacing of values of l over which Bessel and transfer functions are sampled (so, spacing becomes linear instead of logarithmic at some point) */

  double hyper_x_min = 1.0e-5; /**< flat case: lower bound on the smallest value of x at which we sample \f$ \Phi_l^{\nu}(x)\f$ or \f$ j_l(x)\f$ */
  double hyper_sampling_flat = 8.0; /**< flat case: number of sampled points x per approximate wavelength \f$ 2\pi \f$, should remain >7.5 */
  double hyper_sampling_curved_low_nu = 7.0; /**< open/closed cases: number of sampled points x per approximate wavelength \f$ 2\pi/\nu\f$, when \f$ \nu \f$ smaller than hyper_nu_sampling_step */
  double hyper_sampling_curved_high_nu = 3.0; /**< open/closed cases: number of sampled points x per approximate wavelength \f$ 2\pi/\nu\f$, when \f$ \nu \f$ greater than hyper_nu_sampling_step */
  double hyper_nu_sampling_step = 1000.0; /**< open/closed cases: value of nu at which sampling changes  */
  double hyper_phi_min_abs = 1.0e-10; /**< small value of Bessel function used in calculation of first point x (\f$ \Phi_l^{\nu}(x) \f$ equals hyper_phi_min_abs) */
  double hyper_x_tol = 1.0e-4; /**< tolerance parameter used to determine first value of x */
  double hyper_flat_approximation_nu = 4000.0; /**< value of nu below which the flat approximation is used to compute Bessel function */
  ...
  double transfer_neglect_delta_k_S_t0 = 0.15; /**< for temperature source function T0 of scalar mode, range of k values (in 1/Mpc) taken into account in transfer function: for l < (k-delta_k)*tau0, ie for k > (l/tau0 + delta_k), the transfer function is set to zero */
  double transfer_neglect_delta_k_S_t1 = 0.04; /**< same for temperature source function T1 of scalar mode */
  double transfer_neglect_delta_k_S_t2 = 0.15; /**< same for temperature source function T2 of scalar mode */
  double transfer_neglect_delta_k_S_e = 0.11; /**< same for polarization source function E of scalar mode */
  double transfer_neglect_delta_k_V_t1 = 1.0; /**< same for temperature source function T1 of vector mode */
  double transfer_neglect_delta_k_V_t2 = 1.0; /**< same for temperature source function T2 of vector mode */
  double transfer_neglect_delta_k_V_e = 1.0; /**< same for polarization source function E of vector mode */
  double transfer_neglect_delta_k_V_b = 1.0; /**< same for polarization source function B of vector mode */
  double transfer_neglect_delta_k_T_t2 = 0.2; /**< same for temperature source function T2 of tensor mode */
  double transfer_neglect_delta_k_T_e = 0.25; /**< same for polarization source function E of tensor mode */
  double transfer_neglect_delta_k_T_b = 0.1; /**< same for polarization source function B of tensor mode */

  double transfer_neglect_late_source = 400.0; /**< value of l below which the CMB source functions can be neglected at late time, excepted when there is a Late ISW contribution */

  double l_switch_limber = 10.; /**< when to use the Limber approximation for project gravitational potential cl's */
  ...
  double l_switch_limber_for_nc_local_over_z = 100.0; /**< when to use the Limber approximation for local number count contributions to cl's (relative to central redshift of each bin) */
  ...
  // reduce to e.g. 30.
  double l_switch_limber_for_nc_los_over_z = 30.0; /**< when to use the Limber approximation for number count contributions to cl's integrated along the line-of-sight (relative to central redshift of each bin) */

  double selection_cut_at_sigma = 5.0; /**< in sigma units, where to cut gaussian selection functions */
  double selection_sampling = 50.0; /**< controls sampling of integral over time when selection functions vary quicker than Bessel functions. Increase for better sampling. */
  double selection_sampling_bessel = 20.0; /**< controls sampling of integral over time when selection functions vary slower than Bessel functions. Increase for better sampling. IMPORTANT for lensed contributions. */
  double selection_sampling_bessel_los = 20.0; /**< controls sampling of integral over time when selection functions vary slower than Bessel functions. This parameter is specific to number counts contributions to Cl integrated along the line of sight. Increase for better sampling */
  double selection_tophat_edge = 0.1; /**< controls how smooth are the edge of top-hat window function (<<1 for very sharp, 0.1 for sharp) */

  /*
   * Nonlinear module precision parameters
   * */

  double sigma_k_per_decade = 80.; /**< logarithmic stepsize controlling the precision of integrals for sigma(R,k) and similar quantitites */
  ...
  double halofit_min_k_nonlinear = 1.0e-4; /**< value of k in 1/Mpc below which non-linear corrections will be neglected */
  ...
  int accurate_lensing = _FALSE_; /**< switch between Gauss-Legendre quadrature integration and simple quadrature on a subdomain of angles */
  int num_mu_minus_lmax = 70; /**< difference between num_mu and l_max, increase for more precision */
  int delta_l_max = 500; /**< difference between l_max in unlensed and lensed spectra */
  double tol_gauss_legendre = DBL_EPSILON; /**< tolerance with which quadrature points are found: must be very small for an accurate integration (if not entered manually, set automatically to match machine precision) */


  /** @name - general precision parameters */
  //@{
  double smallest_allowed_variation = DBL_EPSILON; /**< machine-dependent, assigned automatically by the code */
  //@}
