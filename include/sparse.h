...
struct sparse_matrix {
  ...
  std::vector<int>
      Ap; /* Ap[0..ncols]. Ap[k+1]-Ap[k] is the number of entries in the k'th column. */
...
struct sparse_numerical {
  ...
  std::vector<int*>
      xi; /*xi[k] points to a row of xi, which holds the topological ordered indices.*/
...
void sp_amd(int* Cp, int* Ci, int n, int cnzmax, int* P, int* W);
