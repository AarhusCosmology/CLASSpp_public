...
void DNCDMSpecies::SetBackgroundInitialConditions(double /*a_rel*/, double* pvecback_integration) {
  ...
  for (int index_q = 0; index_q < ncdm_->q_size_ncdm_[ncdm_id_]; index_q++) {
    ...
    pvecback_integration[index_bi_dlnfdlnq_separate_decay_ + index_q] = -q*std::exp(q)/(std::exp(q) + 1); // Fermi-Dirac
  }
}
...
void DNCDMSpecies::BackgroundDerivs(double /*tau*/, const double* /*y*/, double* dy,
  ...
  for (int i = 0; i < ncdm_->q_size_ncdm_[ncdm_id_]; ++i) {
    ...
    dy[index_bi_dlnfdlnq_separate_decay_ + i] = a * a * M_ncdm * Gamma * q * q / std::pow(epsilon, 3);
  }
}

// ── Perturbations ──────────────────────────────────────────────────────────

void DNCDMSpecies::RegisterPerturbationIndices(perturb_vector* pv, const precision* ppr, int& index_pt,
...
double DNCDMSpecies::Delta(const perturb_vector* pv, const double* y, const double* pvecback, const perturb_workspace* ppw) const {
  if (pv->index_ncdm_.at(ncdm_id_).empty()) return 0.;

  const double a = ppw->scalar_ctx.a;
  double rho_delta_ncdm = 0.0;
  for (int iq = 0; iq < pv->q_size_ncdm[ncdm_id_]; ++iq) {
    double w0 = std::exp(pvecback[index_bg_lnf_decay_dr1_ + iq]) * ncdm_->decay_dr_map_.at(ncdm_id_).dq[iq];
    const double q = ncdm_->q_ncdm_[ncdm_id_][iq];
...
double DNCDMSpecies::Theta(const perturb_vector* pv, const double* y, const double* pvecback, const perturb_workspace* ppw) const {
  if (pv->index_ncdm_.at(ncdm_id_).empty()) return 0.;
...
double DNCDMSpecies::DeltaP(const perturb_vector* pv, const double* y, const double* pvecback, const perturb_workspace* ppw) const {
  if (pv->index_ncdm_.at(ncdm_id_).empty()) return 0.;
...
double DNCDMSpecies::RhoPlusPShear(const perturb_vector* pv, const double* y, const double* pvecback, const perturb_workspace* ppw) const {
  if (pv->index_ncdm_.at(ncdm_id_).empty()) return 0.;
