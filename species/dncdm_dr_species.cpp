...
DNCDM_DR_Species::DNCDM_DR_Species(int ncdm_id, std::shared_ptr<NonColdDarkMatter> ncdm,
...
void DNCDM_DR_Species::BackgroundDerivs(double tau, const double* y,
...
void DNCDM_DR_Species::AddCouplingDerivs(double /*tau*/, const double* y, double* dy,
  ...
  const double rprime_dr = std::pow(a, 5) / (pba_->H0 * pba_->H0) * M_ncdm * Gamma * pvecback[dncdm_->bg_number_index()];
  ...
  auto ComputeFl = [&](int index_q, int lmax, std::vector<double>& output) {
    ...
    if (x < 0.9999) {
      int km = 42 + lmax;
      if (x > 0.9) km *= int(-1.0 - 1.8*log(1./x - 1.0));
      double Fp2 = 0.;
      double Fp1 = 1.;
      for (int l = km; l >= 0; --l) {
        ...
        if ((Fp > 1e200) || (l == 0)) {
          Fp1 /= Fp;
          for (int ll = l + 1; ll <= lmax; ++ll) output[ll*q_size + index_q] /= Fp;
          Fp = 1.0;
        }
        if (l <= lmax) output[l*q_size + index_q] = Fp;
        Fp2 = Fp1;
        Fp1 = Fp;
      }
    } else {
      output[0*q_size + index_q] = 1.;
      if (lmax > 0) output[1*q_size + index_q] = x;
      if (lmax > 1) output[2*q_size + index_q] = (x*(5.*x*x - 3.) + 3.*pow(x*x - 1.,2.)*atanh(x))/(2.*x*x*x);
  ...
  auto compute_collision_integral = [&](int l) {
    ...
    if (ppw->approx[ppw->index_ap_ncdmfa] == (int) ncdmfa_off) {
      ...
      for (int index_q = 0; index_q < q_size; ++index_q) {
        ...
        if (w0 == 0.) { must_rescale = true; break; }
      ...
      if (must_rescale) {
        integral_num = 0.; integral_denom = 0.;
      ...
      if (l == 0) return rprime_dr*y[pv->index_ncdm_.at(ncdm_id_)[0]];
      else if (l == 1) return rprime_dr*y[pv->index_ncdm_.at(ncdm_id_)[0] + 1]/k;
      else return 0.;
    }
  };
