...
void BaryonsSpecies::ApplyInitialConditions(double* y, const PerturbIcContext& ctx) {
  ...
  if (pv->index_pt_delta_b < 0 || pv->index_pt_theta_b < 0) return;
...
void BaryonsSpecies::FillSources(const double* y, const double* /*dy*/,
  ...
  if (p_mod->has_source_delta_b_ == _TRUE_) {
    p_mod->SetSourceValue(ctx.index_md, ctx.index_ic, p_mod->index_tp_delta_b_,
  ...
  if (p_mod->has_source_theta_b_ == _TRUE_) {
    p_mod->SetSourceValue(ctx.index_md, ctx.index_ic, p_mod->index_tp_theta_b_,
                          ctx.index_tau, ctx.index_k,
                          y[pv->index_pt_theta_b]
