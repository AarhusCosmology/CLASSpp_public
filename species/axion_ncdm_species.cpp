...
AxionNCDMSpecies::AxionNCDMSpecies(FileContent* pfc,
  ...
  class_test_severe(T_opt && gstar_opt,
                    "axion species '%s': give either T or gstar_dec, not both",
                    instance_name.c_str());
  class_test_severe(!T_opt && !gstar_opt,
                    "axion species '%s': one of T (= T_axion/T_cmb) or gstar_dec is required",
                    instance_name.c_str());
  class_test(T_opt && *T_opt <= 0.,
             "axion species '%s': T (= T_axion/T_cmb) must be positive",
             instance_name.c_str());
  if (gstar_opt) {
    constexpr double kGstarSToday = 43. / 11.;  // 2 + (7/8)*6*(4/11)
    class_test(*gstar_opt <= kGstarSToday,
               "axion species '%s': gstar_dec must exceed today's g*S = 43/11; the axion cannot "
               "be hotter than the photons it decoupled from",
               instance_name.c_str());
    T_ = std::cbrt(kGstarSToday / *gstar_opt);
  }

  class_test(ksi_ != 0.,
             "axion species '%s': chemical potential ksi is not supported (Bose-Einstein with mu "
             "> 0 is ill-defined)",
             instance_name.c_str());
  class_test_severe(input.get_or("use_psd_file", 0) != 0,
                    "axion species '%s': use_psd_file is incompatible — the Bose-Einstein PSD is "
