...
std::shared_ptr<NonColdDarkMatter> NonColdDarkMatter::Create(FileContent* pfc,
...
int NonColdDarkMatter::background_ncdm_distribution(void* pbadist, double q, double* f0) {
  background_parameters_for_distributions* pbadist_local =
      static_cast<background_parameters_for_distributions*>(pbadist);
  ...
  if (pbadist_local->ncdm->got_files_[n_ncdm] == _TRUE_) {
    ...
    if (q < pbadist_local->q[0]) {
      ...
      *f0 = f0last * exp(-(qlast - q) * df0last / f0last / dqlast);
      ...
      //Do interpolation:
      class_call(array_interpolate_spline(pbadist_local->q.data(),
    ...
    *f0 = 1.0 / pow(2 * _PI_, 3) * (1. / (exp(q - ksi) + 1.) + 1. / (exp(q + ksi) + 1.));
    ...
    if (_FALSE_) {
      ...
      class_test(param == NULL,
                 pbadist_local->ncdm->error_message_,
                 "Analytic expression wants to use 'ncdm_psd_parameters', but they have not been "
                 "entered!");
      ...
      mixing_matrix[1][0] = pow(fabs(sqrt((1 - square_s12) * square_s13 * square_s23) +
                                     sqrt(square_s12 * (1 - square_s23))),
                                2);
      mixing_matrix[1][1] = pow(fabs(sqrt(square_s12 * square_s23 * square_s13) -
                                     sqrt((1 - square_s12) * (1 - square_s23))),
                                2);
      mixing_matrix[1][2] = pow(fabs(sqrt(square_s23 * (1 - square_s13))), 2);
      mixing_matrix[2][0] = pow(fabs(sqrt(square_s12 * square_s23) -
                                     sqrt((1 - square_s12) * square_s13 * (1 - square_s23))),
                                2);
      mixing_matrix[2][1] = pow(sqrt((1 - square_s12) * square_s23) +
                                    sqrt(square_s12 * square_s13 * (1 - square_s23)),
                                2);
      ...
      *f0 = 0.0;
      for (int i = 0; i < 3; i++) {
        *f0 += mixing_matrix[i][n_ncdm] * 1.0 / pow(2 * _PI_, 3) *
               (1. / (exp(q - pbadist_local->ncdm->ksi_ncdm_[i]) + 1.) +
                1. / (exp(q + pbadist_local->ncdm->ksi_ncdm_[i]) + 1.));
      }
...
int NonColdDarkMatter::background_ncdm_test_function(void* pbadist, double q, double* test) {
...
int NonColdDarkMatter::background_ncdm_init(FileContent* pfc, const NcdmSettings& ncdm_settings) {
  ...
  if ((N_ncdm_standard_ < 0) || (N_ncdm_decay_dr_ < 0)) {
    throw std::invalid_argument(
        "Amount of ncdm species must be positive, please check the N_ncdm variables in your input "
        "file.");
  }
  ...
  auto read_list_of_ints_with_deprecated = [&](const std::string& varname,
    ...
    std::vector<int> raw;
    class_call(readIntegerList(pfc, varname.c_str(), raw, &flg1, error_message_),
               error_message_,
               error_message_);
    std::vector<int> raw2;
    class_call(readIntegerList(pfc, varname_deprec.c_str(), raw2, &flg2, error_message_),
               error_message_,
               error_message_);
  ...
  auto read_list_of_doubles_with_deprecated = [&](const std::string& varname,
    ...
    class_call(readDoubleList(pfc, varname.c_str(), raw, &flg1, error_message_),
               error_message_,
               error_message_);
    std::vector<double> raw2;
    class_call(readDoubleList(pfc, varname_deprec.c_str(), raw2, &flg2, error_message_),
               error_message_,
               error_message_);
    if ((flg1 == _TRUE_) && (flg2 == _TRUE_)) {
      throw std::invalid_argument(std::string("In input file, you can only enter one of ") +
  ...
  if (N_ncdm_decay_dr_ > 0) {
    auto extend_list_of_integers =
        [&](const std::string& key, std::vector<int>& output, int default_value = 0) {
          ...
          class_test(static_cast<int>(tmp.size()) != N_ncdm_decay_dr_,
                     errmsg,
    ...
    auto extend_list_of_doubles =
        [&](const std::string& key, std::vector<double>& output, double default_value = 0.0) {
          output.resize(N_ncdm_standard_ + N_ncdm_decay_dr_);
    ...
    for (int n = 0; n < N_ncdm_; n++) {
      class_test((ncdm_input_q_size_[n] > 95) && (ncdm_quadrature_strategy_[n] == 0),
                 errmsg,
                 "Currently, only quadrature strategy 3 is compatible with Number of momentum bins "
                 "larger than 95.");
    }
  }
  ...
  {
    int found = _FALSE_;
    class_call(readIntegerList(pfc, "use_ncdm_psd_files", got_files_, &found, errmsg),
               errmsg,
               errmsg);
    if (found == _FALSE_) {
      got_files_.assign(N_ncdm_, _FALSE_);
    }
    class_test(static_cast<int>(got_files_.size()) != N_ncdm_,
               errmsg,
  ...
  if (flag1 == _TRUE_) {
    ...
    if (fileentries > 0) {
      ...
      for (const std::string& file : raw_psd_files) {
        FileArg entry = {};
        class_test(file.size() >= _ARGUMENT_LENGTH_MAX_,
                   errmsg,
                   "Filename '%s' too long; increase _ARGUMENT_LENGTH_MAX_.",
                   file.c_str());
  ...
  {
    class_call(readDoubleList(pfc, "ncdm_psd_parameters", ncdm_psd_parameters_, &flag1, errmsg),
               errmsg,
               errmsg);
    if (flag1 == _FALSE_) {
  ...
  if (N_ncdm_decay_dr_ > 0) {
    ...
    int size = 0;
    class_call(readDoubleList(pfc, "Gamma_ncdm_decay_dr", Gamma_list, &Gamma_read, errmsg),
               errmsg,
               errmsg);
    class_call(readDoubleList(pfc,
               ...
               errmsg,
               errmsg);
    class_call(readDoubleList(pfc, "lifetime_ncdm_decay_dr", lifetime_list, &lifetime_read, errmsg),
               errmsg,
               errmsg);
    class_call(readDoubleList(pfc,
    ...
    if (Gamma_read == _TRUE_)
      size = static_cast<int>(Gamma_list.size());
    if (log10Gamma_read == _TRUE_)
      size = static_cast<int>(log10Gamma_list.size());
    if (lifetime_read == _TRUE_)
      size = static_cast<int>(lifetime_list.size());
    if (log10lifetime_read == _TRUE_)
      size = static_cast<int>(log10lifetime_list.size());

    if (class_at_least_two_of_four(Gamma_read,
                                   log10Gamma_read,
                                   lifetime_read,
                                   log10lifetime_read)) {
      throw std::invalid_argument(
          "More than two of the following inputs are given. Choose one! \nGamma_ncdm_decay_dr, "
          "log10Gamma_ncdm_decay_dr, lifetime_ncdm_decay_dr, log10lifetime_ncdm_decay_dr.");
    }
    if (size != N_ncdm_decay_dr_) {
      throw std::invalid_argument(
          "Number of entries (" + std::to_string(size) +
          ") in either Gamma_ncdm_decay_dr, log10Gamma_ncdm_decay_dr, lifetime_ncdm_decay_dr or "
          "log10lifetime_ncdm_decay_dr does not match N_ncdm_decay_dr (" +
          std::to_string(N_ncdm_decay_dr_) + ".");
    }
    ...
    for (int n = 0; n < N_ncdm_decay_dr_; n++) {
      if (Gamma_read == _TRUE_) {
        ...
        Gamma_input[n] = 1. / lifetime_list[n] / (365 * 24 * 60 * 60) * _Mpc_over_m_ *
                         1e-3;  // Conversion from yr^-1 to km/s/Mpc
      }
  ...
  for (int k = 0; k < N_ncdm_; k++) {
    ...
    if ((!got_files_.empty()) && (got_files_[k] == _TRUE_)) {
      ...
      class_test(psdfile == NULL,
                 error_message_,
                 "Could not open file %s!",
                 ncdm_psd_files_.data() + filenum * _ARGUMENT_LENGTH_MAX_);
      // Find size of table:
      double tmp1;
      double tmp2;
      int status = 0;
    ...
    if (ncdm_quadrature_strategy_[k] == qm_auto) {
      ...
      if (ncdm_types_[k] == NCDMType::decay_dr) {
        for (int index_q = 0; index_q < q_size_ncdm_bg_[k]; index_q++) {
          double f0;
          class_call(background_ncdm_distribution(&pbadist, q_ncdm_bg_[k][index_q], &f0),
                     error_message_,
                     error_message_);
    ...
    for (int index_q = 0; index_q < q_size_ncdm_[k]; index_q++) {
      ...
      double f0;
      class_call(background_ncdm_distribution(&pbadist, q, &f0), error_message_, error_message_);
      ...
      if (fabs(f0) == 0.)
        dlnf0_dlnq_ncdm_[k][index_q] =
            -q; /* valid for whatever f0 with exponential tail in exp(-q) */
      else
        dlnf0_dlnq_ncdm_[k][index_q] = q / f0 * df0dq;
    }

    factor_ncdm_[k] = deg_ncdm_[k] * 4 * _PI_ * pow(ncdm_settings.T_cmb * T_ncdm_[k] * _k_B_, 4) *
                      8 * _PI_ * _G_ / 3. / pow(_h_P_ / 2. / _PI_, 3) / pow(_c_, 7) * _Mpc_over_m_ *
                      _Mpc_over_m_;

    rho_nu_rel_ = 56.0 / 45.0 * pow(_PI_, 6) * pow(4.0 / 11.0, 4.0 / 3.0) * _G_ / pow(_h_P_, 3) /
                  pow(_c_, 7) * pow(_Mpc_over_m_, 2) * pow(ncdm_settings.T_cmb * _k_B_, 4);
    ...
    if (ncdm_types_[k] == NCDMType::decay_dr) {
      ...
      if (ncdm_quadrature_strategy_[k] != 3) {
        throw std::invalid_argument(
            "DNCDM currently only admits quadrature strategy 3. Please change your input "
            "accordingly.\n");
      }
      else {
...
int NonColdDarkMatter::background_ncdm_momenta(int n_ncdm,
                                               double z,
                                               double* n,
                                               double* rho,
                                               double* p,
...
int NonColdDarkMatter::background_ncdm_momenta_mass(int n_ncdm,
                                                    double M,
                                                    double z,
                                                    double* n,
                                                    double* rho,
...
*/
int NonColdDarkMatter::background_ncdm_momenta_deg(int n_ncdm,
                                                   double deg,
                                                   double z,
                                                   double T_cmb,
                                                   double* n,
  ...
  double factor2 = deg * 4 * _PI_ * pow(T_cmb * T_ncdm_[n_ncdm] * _k_B_, 4) * 8 * _PI_ * _G_ / 3. /
...
double NonColdDarkMatter::background_ncdm_M_from_Omega(int n_ncdm,
                                                       double H0,
                                                       double Omega0,
                                                       double tol_M_ncdm) {
  int maxiter = 50;
  ...
  class_test(rho0 < rho,
             error_message_,
             "The value of Omega for the %dth species, %g, is less than for a massless species! It "
             "should be at least %g. Check your input.",
             n_ncdm,
             Omega0,
             Omega0 * rho / rho0);
...
void NonColdDarkMatter::PrintNeffInfo() const {
  ...
  for (int n_ncdm = 0; n_ncdm < N_ncdm_; ++n_ncdm) {
    ...
    printf(
        " -> ncdm species i=%d sampled with %d (resp. %d) points for purpose of background (resp. "
        "perturbation) integration. In the relativistic limit it gives Delta N_eff = %g\n",
        n_ncdm + 1,
...
std::tuple<double, double> NonColdDarkMatter::GetRescaledParameters(int n_ncdm,
                                                                    double a,
...
void NonColdDarkMatter::SetDegAndFactor(int n_ncdm, double deg, double T_cmb) {
  ...
  factor_ncdm_[n_ncdm] = deg_ncdm_[n_ncdm] * 4 * _PI_ * pow(T_cmb * T_ncdm_[n_ncdm] * _k_B_, 4) *
                         8 * _PI_ * _G_ / 3. / pow(_h_P_ / 2. / _PI_, 3) / pow(_c_, 7) *
                         _Mpc_over_m_ * _Mpc_over_m_;
}

void NonColdDarkMatter::SetDeg_from_Omega_ini(
    int n_ncdm, double z_ini, double H0, double Omega_ini, double T_cmb) {
  double rho_deg1;
...
double NonColdDarkMatter::GetIni(double a, double a_today, double tol_ncdm_initial_w) const {
  ...
  for (counter = 0; counter < _MAX_IT_; counter++) {
    ...
    for (int n_ncdm = 0; n_ncdm < N_ncdm_; n_ncdm++) {
      class_call(background_ncdm_momenta(n_ncdm,
                                         a_today / a - 1.0,
                                         NULL,
                                         &rho_ncdm,
                                         &p_ncdm,
  ...
  class_test(counter == _MAX_IT_,
             error_message_,
             "Search for initial scale factor a such that all ncdm species are relativistic "
             "failed.");
  return a;
}
